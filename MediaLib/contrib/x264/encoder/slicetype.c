...
#define TRY_BIDIR( mv0, mv1, penalty ) \
    ...
    if( b_bidir )
    {
        ...
        CLIP_MV( dmv[1] );
        if( h->param.analyse.i_subpel_refine <= 1 )
            M64( dmv ) &= ~UINT64_C(0x0001000100010001); /* mv & ~1 */
...
#endif
        {
            ...
            if( h->param.i_lookahead_threads > 1 )
            {
                ...
                for( i = 0; i < h->param.i_lookahead_threads; i++ )
                {
                    ...
                    int thread_height,thread_output_size;
                    ...
                    {
                        x264_slicetype_slice_t slice_t = { t, a, frames, p0, p1, b, dist_scale_factor, do_search, w,
