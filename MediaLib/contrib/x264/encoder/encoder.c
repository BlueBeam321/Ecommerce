...
static int x264_validate_parameters( x264_t *h, int b_open )
{
    ...
    if( h->param.i_avcintra_class )
    {
        ...
        fps_num = h->param.i_fps_num, fps_den = h->param.i_fps_den;
...
x264_t *x264_encoder_open( x264_param_t *param )
{
	...
	if (h->param.b_frame_converter)
	{	/* PMG */
		h->b_frame_converter = 1;
	}
...
int x264_encoder_headers( x264_t *h, x264_nal_t **pp_nal, int *pi_nal )
{
    ...
    /* identify ourselves */
    //x264_nal_start( h, NAL_SEI, NAL_PRIORITY_DISPOSABLE ); // commented by WMI on 2019/04/10
...
//            x264_log( h, X264_LOG_WARNING, "non-strictly-monotonic PTS\n" );
...
#if defined(_VS2010_) || defined(__GNUC__)
    ...
    if( h->fenc->b_keyframe )
    {
        ...
        if( h->param.b_repeat_headers && h->fenc->i_frame == 0 && !h->param.i_avcintra_class )
        {
            /* identify ourself */
            //x264_nal_start( h, NAL_SEI, NAL_PRIORITY_DISPOSABLE );
            //if( x264_sei_version_write( h, &h->out.bs ) ) // commented by WMI on 2019/04/10
            //    return -1;
            //if( x264_nal_end( h ) )
            //    return -1;
            //overhead += h->out.nal[h->out.i_nal-1].i_payload + SEI_OVERHEAD;
                                   ...
                                   x264_picture_t *pic_out )
{
	...
	if (h->b_frame_converter)
	{
		pic_out->i_chapter	= h->fenc->i_chapter;
		pic_out->i_file_pos = h->fenc->i_file_pos;
	}
