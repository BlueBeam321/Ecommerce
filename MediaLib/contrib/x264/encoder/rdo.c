...
#define TRELLIS_SCORE_MAX INT64_C(-1) // negative marks the node as invalid
#define TRELLIS_SCORE_BIAS INT64_C(1)<<60; // bias so that all valid scores are positive, even after negative contributions from psy
...
#if defined(WIN32) && !defined(_VS2010_)
#define COEF(const_level, ctx_hi, j, node_ctx, level1_ctx, levelgt1_ctx)\
    if( !j || (int64_t)nodes_prev[j].score >= 0 )\
        levels_used = trellis_coef( j, const_level, abs_level, prefix, suffix_cost, node_ctx, level1_ctx, levelgt1_ctx,\
...
\
        ...
        {\
            ...
            for( j = 1; j < (ctx_hi?8:4); j++ )\
                SET_LEVEL( nodes_cur[j], nodes_cur[j], 0 );\
            continue;\
        }\
\
        sign_coef = orig_coefs[zigzag[i]];\
        abs_coef = abs( sign_coef );\
        q = abs( quant_coefs[i] );\
                         ...
                         int b_chroma, int dc, int num_coefs, int idx, int b_8x8 )
{
    ...
    while( 1 )
    {
        ...
        for( i = b_ac; i <= last_nnz; i++ )
        {
            int cur_round,round_change,old_coef,new_coef,cur_mask,cur_distortion_delta,cur_score;
