...
static int get_plane_ptr1( x264_t *h, x264_picture_t *src, uint8_t **pix, int *stride, int plane, int xshift, int yshift )
{
...
#define get_plane_ptr(h, src, pix, stride, plane, xshift, yshift) do{ if( get_plane_ptr1(h, src, pix, stride, plane, xshift, yshift) < 0 ) return -1; }while(0)
#else
#define get_plane_ptr(...) do{ if( get_plane_ptr1(__VA_ARGS__) < 0 ) return -1; }while(0)
#endif
...
int x264_frame_copy_picture( x264_t *h, x264_frame_t *dst, x264_picture_t *src )
{
	...
	if (h->b_frame_converter)
	{/* PMG */
		dst->i_chapter = src->i_chapter;
		dst->i_file_pos = src->i_file_pos;
	}
