...
static int handle_opts( const char * const *optlist, char **opts, video_info_t *info, resizer_hnd_t *h )
{
    ...
    if( fittobox )
    {
        ...
        {
            int new_width = x264_round( height * height_units / (width_units * csp->mod_width) );
            new_width *= csp->mod_width;
            width = X264_MIN( new_width, width );
        }
        else
        {
            int new_height = x264_round( width * width_units / (height_units * csp->mod_height) );
    ...
    else
    {
        ...
        else if( str_sar ) /* sar only -> adjust res */
        {
             ...
             {
                 width = x264_round( info->width * height_units / (width_units * csp->mod_width) );
                 width *= csp->mod_width;
             }
             else // SAR got thinner, decrease height
             {
                 height = x264_round( info->height * width_units / (height_units * csp->mod_height) );
