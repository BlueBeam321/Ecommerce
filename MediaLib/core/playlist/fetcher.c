...
#include "common/core/playlist/art.h"
#include "fetcher.h"
#include "common/core/input/input_interface.h"
#include "common/core/misc/background_worker.h"
...
static char* CreateCacheKey( input_item_t* item )
{
     ...
     * { dog, stick } */
    if( !artist || !album || asprintf( &key, "%s:%d:%s:%d",
    ...
    struct background_worker* bg, struct fetcher_request* req, void** handle )
{
    ...
    if (!vlc_clone(&th->thread, FetcherThread, th, "fetcher", VLC_THREAD_PRIORITY_LOW))
    {
        *handle = th;
        return VLC_SUCCESS;
    }
