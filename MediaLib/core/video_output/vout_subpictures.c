    ...
    int *attachment_count)
{
    ...
    if (spu->p->input)
        ret = input_Control((input_thread_t*)spu->p->input, INPUT_GET_ATTACHMENTS, attachment_ptr, attachment_count);
    return ret;
}
...
{
    return spu_scale_create(wn * SCALE_UNIT / wd, hn * SCALE_UNIT / hd);
}
    ...
    mtime_t render_date)
{
    ...
    *dst_ptr = NULL;
    ...
    {
        SpuRenderText(spu, &restore_text, region, chroma_list, render_date - subpic->i_start);
    ...
    SpuRegionPlace(&x_offset, &y_offset, subpic, region);
    ...
    *dst_area = spu_area_create(x_offset, y_offset,
    ...
    {
        if (!subpic->b_absolute)
            SpuAreaFixOverlap(dst_area, subtitle_area, subtitle_area_count, region->i_align);
        else if (!region->p_text)// Added by K.C.Y
        {
            bool overlaped = false;
            spu_area_t a = spu_area_scaled(*dst_area);
            for (int i = 0; i < subtitle_area_count; i++)
            {
                spu_area_t sub = spu_area_scaled(subtitle_area[i]);
                if (spu_area_overlap(a, sub))
                {
                    overlaped = true;
                    break;
        ...
        using_palette || convert_chroma))
    {
        ...
        if (!region->p_private && dst_width > 0 && dst_height > 0)
        {
                ...
                (convert_chroma && !using_palette)))
            {
                ...
                if (convert_chroma)
                    scale->fmt_out.i_codec = scale->fmt_out.video.i_chroma = chroma_list[0];

                scale->fmt_out.video.i_width = dst_width;
                scale->fmt_out.video.i_height = dst_height;

                scale->fmt_out.video.i_visible_width = spu_scale_w(region->fmt.i_visible_width, scale_size);
                scale->fmt_out.video.i_visible_height = spu_scale_h(region->fmt.i_visible_height, scale_size);
    ...
    if (dst)
    {
        ...
        if (subpic->b_fade)
        {
            ...
            if (fade_start <= render_date && fade_start < subpic->i_stop)
                fade_alpha = 255 * (subpic->i_stop - render_date) / (subpic->i_stop - fade_start);
        }
    ...
    mtime_t render_osd_date)
{
    ...
    for (unsigned int index = 0; index < i_subpicture; index++)
    {
            ...
            subpic->i_original_picture_height <= 0)
        {
            if (subpic->i_original_picture_width > 0 ||
                subpic->i_original_picture_height > 0)
                msg_Err(spu, "original picture size %dx%d is unsupported", subpic->i_original_picture_width, subpic->i_original_picture_height);
            else
...
void spu_Destroy(spu_t *spu)
{
    ...
    if (sys->vout)
        filter_chain_ForEach(sys->source_chain, SubSourceDelProxyCallbacks, sys->vout);
    filter_chain_Delete(sys->source_chain);
    free(sys->source_chain_current);
    if (sys->vout)
        filter_chain_ForEach(sys->filter_chain, SubFilterDelProxyCallbacks, sys->vout);
...
void spu_PutSubpicture(spu_t *spu, subpicture_t *subpic)
{
    ...
    if (chain_update)
    {
        if (*chain_update)
        {
            ...
            if (sys->vout)
                filter_chain_ForEach(sys->filter_chain, SubFilterAddProxyCallbacks, sys->vout);
        }
        else
            filter_chain_Reset(sys->filter_chain, NULL, NULL);
                         ...
                         bool ignore_osd)
{
    ...
    if (chain_update)
    {
        ...
        if (sys->vout)
            filter_chain_ForEach(sys->source_chain, SubSourceAddProxyCallbacks, sys->vout);
    ...
    if (!chroma_list || *chroma_list == 0)
        chroma_list = vlc_fourcc_IsYUV(fmt_dst->i_chroma) ? chroma_list_default_yuv : chroma_list_default_rgb;
    ...
    SpuSelectSubpictures(spu, &subpicture_count, subpicture_array, render_subtitle_date, render_osd_date, ignore_osd);
