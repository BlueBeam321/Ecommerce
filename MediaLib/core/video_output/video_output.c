...
static vout_thread_t *VoutCreate(vlc_object_t *object, const vout_configuration_t *cfg)
{
    ...
    /* Take care of some "interface/control" related initializations */
    vout_IntfInit(vout);
    ...
    /* */
    if (vlc_clone(&vout->p->thread, Thread, vout, "video_output", VLC_THREAD_PRIORITY_OUTPUT))
    {
...
void vout_GetResetStatistic(vout_thread_t *vout, unsigned * displayed, unsigned * lost)
{
...
{
    vout_control_PushInteger(&vout->p->control, VOUT_CONTROL_FLUSH_SUBPICTURE, channel);
}
...
{
    vout_control_PushBool(&vout->p->control, VOUT_CONTROL_FULLSCREEN, fullscreen);
}
...
{
    vout_control_PushBool(&vout->p->control, VOUT_CONTROL_DISPLAY_FILLED, is_filled);
}
void vout_ControlChangeZoom(vout_thread_t *vout, int num, int den)
{
    vout_control_PushPair(&vout->p->control, VOUT_CONTROL_ZOOM, num, den);
}
void vout_ControlChangeSampleAspectRatio(vout_thread_t *vout, unsigned num, unsigned den)
{
    vout_control_PushPair(&vout->p->control, VOUT_CONTROL_ASPECT_RATIO, num, den);
}
void vout_ControlChangeCropRatio(vout_thread_t *vout, unsigned num, unsigned den)
{
    vout_control_PushPair(&vout->p->control, VOUT_CONTROL_CROP_RATIO, num, den);
}
...
{
    vout_control_PushString(&vout->p->control, VOUT_CONTROL_CHANGE_FILTERS, filters);
}
void vout_ControlChangeSubSources(vout_thread_t *vout, const char *filters)
{
    vout_control_PushString(&vout->p->control, VOUT_CONTROL_CHANGE_SUB_SOURCES, filters);
}
void vout_ControlChangeSubFilters(vout_thread_t *vout, const char *filters)
{
    vout_control_PushString(&vout->p->control, VOUT_CONTROL_CHANGE_SUB_FILTERS, filters);
}
void vout_ControlChangeSubMargin(vout_thread_t *vout, int margin)
{
    vout_control_PushInteger(&vout->p->control, VOUT_CONTROL_CHANGE_SUB_MARGIN, margin);
}
...
{
    filter_DelProxyCallbacks((vlc_object_t *)opaque, filter, FilterRestartCallback);
    return VLC_SUCCESS;
}
...
static void ThreadDelAllFilterCallbacks(vout_thread_t *vout)
{
    ...
    filter_chain_ForEach(vout->p->filter.chain_interactive, ThreadDelFilterCallbacks, vout);
}
    ...
    bool is_locked)
{
    ...
    for (int a = 0; a < 2; a++)
    {
        ...
        for (size_t i = 0; i < vlc_array_count(array); i++)
        {
            ...
            filter_t *filter = filter_chain_AppendFilter(chain, e->name, e->cfg, NULL, NULL);
            if (!filter)
            {
    ...
    if (!es_format_IsSimilar(p_fmt_current, &fmt_target))
    {
        ...
        if (filter_chain_AppendConverter(vout->p->filter.chain_interactive, p_fmt_current, &fmt_target) != 0)
        {
...
static int ThreadDisplayRenderPicture(vout_thread_t *vout, bool is_forced)
{
    ...
    if (do_dr_spu)
    {
        ...
        if (fmt_spu.i_width * fmt_spu.i_height < place.width * place.height)
        {
            ...
            fmt_spu.i_width = fmt_spu.i_visible_width = place.width;
            fmt_spu.i_height = fmt_spu.i_visible_height = place.height;
        }
...
{
    vout_SetDisplayCrop(vout->p->display.vd, 0, 0, x, y, width, height);
}
static void ThreadExecuteCropBorder(vout_thread_t *vout, unsigned left, unsigned top, unsigned right, unsigned bottom)
{
    ...
    vout_SetDisplayCrop(vout->p->display.vd, 0, 0, left, top, -(int)right, -(int)bottom);
}

static void ThreadExecuteCropRatio(vout_thread_t *vout, unsigned num, unsigned den)
{
    vout_SetDisplayCrop(vout->p->display.vd, num, den, 0, 0, 0, 0);
}
...
static int ThreadStart(vout_thread_t *vout, vout_display_state_t *state)
{
    ...
    vout->p->filter.chain_static = filter_chain_NewVideo(vout, true, &owner);

    owner.video.buffer_new = VoutVideoFilterInteractiveNewPicture;
    vout->p->filter.chain_interactive = filter_chain_NewVideo( vout, true, &owner );
