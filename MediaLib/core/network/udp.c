...
extern int net_Socket(vlc_object_t *p_this, int i_family, int i_socktype, int i_protocol);
...
static int net_SetupDgramSocket(vlc_object_t *p_obj, int fd, const struct addrinfo *ptr)
{
    ...
    {
        setsockopt(fd, SOL_SOCKET, SO_RCVBUF, (void *)&(int){ 0x80000 }, sizeof (int));
    }
    if (h_Network)
...
static int net_ListenSingle(vlc_object_t *obj, const char *host, int port, int protocol)
{
    ...
    if (val)
    {
        msg_Err(obj, "Cannot resolve %s port %d : %s", host, port, gai_strerror(val));
        return -1;
    }
    ...
    {
        int fd = net_Socket (obj, ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol);
        if (fd == -1)
        {
...
static int net_SetMcastOut(vlc_object_t *p_this, int fd, int family, const char *iface)
{
    ...
    case AF_INET6:
        if (setsockopt(fd, SOL_IPV6, IPV6_MULTICAST_IF, &scope, sizeof(scope)) == 0)
            return 0;
        break;
#endif
    ...
    case AF_INET:
    {
        ...
        if (setsockopt (fd, SOL_IP, IP_MULTICAST_IF, &req, sizeof (req)) == 0)
            return 0;
        break;
    }
#endif
    ...
    msg_Err(p_this, "cannot force multicast interface %s: %s", iface, vlc_strerror_c(errno));
    return -1;
}
 ...
 */
static int net_SourceSubscribe (vlc_object_t *obj, int fd,
    ...
    const struct sockaddr *grp, socklen_t grplen)
{
    ...
    if (setsockopt(fd, level, MCAST_JOIN_SOURCE_GROUP, &gsr, sizeof(gsr)) == 0)
        return 0;
...
int net_Subscribe(vlc_object_t *obj, int fd, const struct sockaddr *grp, socklen_t grplen)
{
    ...
    case AF_INET6:
    {
        ...
        if (!setsockopt(fd, SOL_IPV6, IPV6_JOIN_GROUP, &ipv6mr, sizeof(ipv6mr)))
            return 0;
        break;
    }
#endif
    ...
    case AF_INET:
    {
        ...
        assert(grplen >= sizeof(struct sockaddr_in));
        imr.imr_multiaddr = ((const struct sockaddr_in *)grp)->sin_addr;
        //setsockopt(fd, SOL_IP, IP_MULTICAST_IF, (char*)&((const struct sockaddr_in *)grp)->sin_addr, sizeof(((const struct sockaddr_in *)grp)->sin_addr));
    ...
    msg_Err(obj, "cannot join multicast group: %s", vlc_strerror_c(net_errno));
    return -1;
}
...
int net_ConnectDgram(vlc_object_t *p_this, const char *psz_host, int i_port, int i_hlim, int proto)
{
    ...
    if (val)
    {
        msg_Err(p_this, "cannot resolve [%s]:%d : %s", psz_host, i_port, gai_strerror(val));
        return -1;
    }
    ...
    for (struct addrinfo *ptr = res; ptr != NULL; ptr = ptr->ai_next)
    {
        ...
        if (errno == ENETUNREACH)
            ...
            msg_Warn(p_this, "%s port %d : %s", psz_host, i_port, vlc_strerror_c(errno));
        net_Close(fd);
    }
    ...
    {
        if (b_unreach)
            msg_Err(p_this, "Host %s port %d is unreachable", psz_host, i_port);
        return -1;
    }
...
int net_OpenDgram(vlc_object_t *obj, const char *psz_bind, int i_bind, const char *psz_server, int i_server, int protocol)
{
    ...
    if (val)
    {
        msg_Err(obj, "cannot resolve %s port %d : %s", psz_server, i_server, gai_strerror(val));
        return -1;
    }
    ...
    if (val)
    {
        msg_Err(obj, "cannot resolve %s port %d : %s", psz_bind, i_bind, gai_strerror(val));
        freeaddrinfo(rem);
        return -1;
    }
...
int net_SetCSCov(int fd, int sendcov, int recvcov)
{
    ...
    if (getsockopt(fd, SOL_SOCKET, SO_TYPE, &type, &(socklen_t){ sizeof(type) }))
        return VLC_EGENERIC;
    ...
    case SOCK_DGRAM: /* UDP-Lite */
        if (sendcov == -1)
            ...
            sendcov += 8; /* partial */
        if (setsockopt(fd, SOL_UDPLITE, UDPLITE_SEND_CSCOV, &sendcov, sizeof (sendcov)))
            return VLC_EGENERIC;
        ...
        if (recvcov == -1)
            ...
            recvcov += 8;
        if (setsockopt(fd, SOL_UDPLITE, UDPLITE_RECV_CSCOV, &recvcov, sizeof (recvcov)))
            return VLC_EGENERIC;
    ...
    case SOCK_DCCP: /* DCCP and its ill-named socket type */
        if ((sendcov == -1) || (sendcov > 56))
            ...
            sendcov = (sendcov + 3) / 4;
        if (setsockopt(fd, SOL_DCCP, DCCP_SOCKOPT_SEND_CSCOV, &sendcov, sizeof (sendcov)))
            return VLC_EGENERIC;
        ...
        if ((recvcov == -1) || (recvcov > 56))
            ...
            recvcov = (recvcov + 3) / 4;
        if (setsockopt(fd, SOL_DCCP, DCCP_SOCKOPT_RECV_CSCOV, &recvcov, sizeof (recvcov)))
            return VLC_EGENERIC;
