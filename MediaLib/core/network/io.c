...
int net_Socket (vlc_object_t *p_this, int family, int socktype,
                int protocol)
{
    int recvBufSize = 0x800000, sndBufSize = 0x800000;
    ...
    setsockopt(fd, SOL_SOCKET, SO_RCVBUF, (char *)&recvBufSize, sizeof(recvBufSize));
 ...
 */
ssize_t (net_Read)(vlc_object_t * obj, int fd,
                   void * buf, size_t len)
{
    size_t rd = 0;
