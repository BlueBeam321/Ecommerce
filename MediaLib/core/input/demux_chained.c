...
static void *vlc_demux_chained_Thread(void *data)
{
    ...
    demux_t *demux = demux_NewAdvanced(dc->fifo, NULL, "", dc->name, "", dc->fifo, dc->out, false);
    if (demux == NULL)
    {
...
vlc_demux_chained_t *vlc_demux_chained_New(vlc_object_t *parent, const char *name, es_out_t *out)
{
...
#if CRYPT_ENABLE
    demux_name = sinjong_decrypt_text(name);
#else
    demux_name= name;
#endif
    vlc_demux_chained_t *dc = malloc(sizeof(*dc) + strlen(demux_name) + 1);
    if (unlikely(dc == NULL))
        return NULL;
    ...
    dc->stats.time = 0;
    dc->out = out;
    strcpy(dc->name, demux_name);
#if CRYPT_ENABLE    
    free(demux_name);
#endif
    vlc_mutex_init(&dc->lock);

    if (vlc_clone(&dc->thread, vlc_demux_chained_Thread, dc, "demux_chained", VLC_THREAD_PRIORITY_INPUT))
    {
        vlc_stream_Delete(dc->fifo);
