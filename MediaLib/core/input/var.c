...
static int EsSpuCallback(vlc_object_t *p_this, char const *psz_cmd,
    vlc_value_t oldval, vlc_value_t newval, void *);
static int EsSpu2Callback(vlc_object_t *p_this, char const *psz_cmd,
    vlc_value_t oldval, vlc_value_t newval, void *);
...
static const vlc_input_callback_t p_input_callbacks[] =
{
    ...
    CALLBACK("spu-es-sub", EsSpu2Callback), // Added by K.C.Y
...
void input_ControlVarInit ( input_thread_t *p_input )
{
    ...
    text.psz_string = _("Subtitle Track(Main)");
    var_Change(p_input, "spu-es", VLC_VAR_SETTEXT, &text, NULL);

    // Added variable by K.C.Y
    var_Create(p_input, "spu-es-sub", VLC_VAR_INTEGER);
    var_Change(p_input, "spu-es-sub", VLC_VAR_SETVALUE, &val, NULL);
    text.psz_string = _("Subtitle Track(Sub)");
    var_Change(p_input, "spu-es-sub", VLC_VAR_SETTEXT, &text, NULL);
    ...
    const vlc_input_callback_t *p_callbacks)
{
    ...
    for (i = 0; p_callbacks[i].psz_name != NULL; i++)
        var_AddCallback(p_input, p_callbacks[i].psz_name, p_callbacks[i].callback, NULL);
}
    ...
    const vlc_input_callback_t *p_callbacks)
{
    int i;
    for (i = 0; p_callbacks[i].psz_name != NULL; i++)
        var_DelCallback(p_input, p_callbacks[i].psz_name, p_callbacks[i].callback, NULL);
}
...
static int EsSpu2Callback(vlc_object_t *p_this, char const *psz_cmd,
    vlc_value_t oldval, vlc_value_t newval, void *p_data) // Added by K.C.Y
{
    input_thread_t *p_input = (input_thread_t*)p_this;
    VLC_UNUSED(psz_cmd); VLC_UNUSED(oldval); VLC_UNUSED(p_data);
    ...
    input_ControlPush(p_input, INPUT_CONTROL_SET_ES2, &newval);
