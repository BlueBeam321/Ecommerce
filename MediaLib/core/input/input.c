...
int input_Start(input_thread_t *p_input)
{
    ...
    priv->is_running = !vlc_clone(&priv->thread, func, priv, "input", VLC_THREAD_PRIORITY_INPUT);
    if (!priv->is_running)
    {
                ...
                *psz_end = ',';
                ...
                while ((psz_end = strchr(psz_start, ',')))
                {
                    *psz_end = 0;
                    ...
                    {
                        p_seekpoint->i_time_offset = atof(psz_start + 5) * CLOCK_FREQ;
                    }
                    psz_start = psz_end + 1;
                }
...
static void MainLoopStatistics( input_thread_t *p_input )
{
    ...
    if (demux_Control(input_priv(p_input)->master->p_demux, DEMUX_GET_POSITION, &f_position))
        f_position = 0.0;
    ...
    if (demux_Control(input_priv(p_input)->master->p_demux, DEMUX_GET_LENGTH, &i_length))
        i_length = 0;
...
static void MainLoop( input_thread_t *p_input, bool b_interactive )
{
    ...
    bool b_pause_after_eof = b_interactive && var_InheritBool( p_input, "play-and-pause" );
    ...
    while( !input_Stopped( p_input ) && input_priv(p_input)->i_state != ERROR_S )
    {
        ...
        if (!strcmp(input_priv(p_input)->master->p_demux->psz_access, "vod")) //by RMS for VOD
            i_wakeup = mdate() + 50000;
        ...
        if( b_paused )
            b_paused = !es_out_GetBuffering( input_priv(p_input)->p_es_out ) || input_priv(p_input)->master->b_eof;
...
static int Init(input_thread_t * p_input)
{
    ...
    /* Create es out */
    priv->p_es_out = input_EsOutTimeshiftNew(p_input, priv->p_es_out_display, priv->i_rate);
...
error:
    ...
    if (input_priv(p_input)->p_resource)
    {
        if (input_priv(p_input)->p_sout)
            input_resource_RequestSout(input_priv(p_input)->p_resource, input_priv(p_input)->p_sout, NULL);
...
static void ControlNav( input_thread_t *p_input, int i_type )
{
    ...
    memset(&vp, 0, sizeof(vlc_viewpoint_t));
    switch (i_type)
    {
        case INPUT_CONTROL_NAV_UP:
            vol_direction = 1;
                     ...
                     int i_type, vlc_value_t val )
{
    ...
    switch( i_type )
    {
        ...
        case INPUT_CONTROL_SET_ES2: // Added by K.C.Y
            /* No need to force update, es_out does it if needed */
            es_out_Control(input_priv(p_input)->p_es_out_display,
                ES_OUT_SET_ES_BY_ID2, (int)val.i_int);
