...
typedef struct es_out_ts_sys_t
{
    input_thread_t *p_input;
    es_out_t       *p_out;
...
static int  CmdInitAdd(ts_cmd_t *, es_out_ts_id_t *, const es_format_t *, bool b_copy);
static void CmdInitSend(ts_cmd_t *, es_out_ts_id_t *, block_t *);
static int  CmdInitDel(ts_cmd_t *, es_out_ts_id_t *);
...
es_out_t *input_EsOutTimeshiftNew( input_thread_t *p_input, es_out_t *p_next_out, int i_rate )
{
    ...
    es_out_ts_sys_t *p_sys = (es_out_ts_sys_t*)malloc(sizeof(*p_sys));
    if( !p_sys )
    {
        free( p_out );
        return NULL;
...
{
    es_out_ts_sys_t *p_sys = (es_out_ts_sys_t*)p_out->p_sys;
...
static es_out_id_t *Add(es_out_t *p_out, const es_format_t *p_fmt)
{
    ...
    es_out_ts_id_t *p_es = (es_out_ts_id_t*)malloc(sizeof(*p_es));
    if (!p_es)
        return NULL;
...
static int Send( es_out_t *p_out, es_out_id_t *p_es, block_t *p_block )
{
    ...
    CmdInitSend(&cmd, (es_out_ts_id_t*)p_es, p_block);
    if (p_sys->b_delayed)
        TsPushCmd(p_sys->p_ts, &cmd);
    else
...
static void Del( es_out_t *p_out, es_out_id_t *p_es )
{
    ...
    CmdInitDel(&cmd, (es_out_ts_id_t*)p_es);
    if (p_sys->b_delayed)
        TsPushCmd(p_sys->p_ts, &cmd);
    else
        CmdExecuteDel(p_sys->p_out, &cmd);

    TAB_REMOVE(p_sys->i_es, p_sys->pp_es, (es_out_ts_id_t*)p_es);
...
static int ControlLocked( es_out_t *p_out, int i_query, va_list args )
{
    ...
    {
        es_out_ts_id_t *p_es = (es_out_ts_id_t*)va_arg(args, es_out_ts_id_t *);
...
static int TsStart( es_out_t *p_out )
{
    ...
    if (vlc_clone(&p_ts->thread, TsRun, p_ts, "es_out_timeshift", VLC_THREAD_PRIORITY_INPUT))
    {
...
static int CmdInitAdd( ts_cmd_t *p_cmd, es_out_ts_id_t *p_es, const es_format_t *p_fmt, bool b_copy )
{
    p_cmd->i_type = C_ADD;
    p_cmd->i_date = mdate();
...
static void CmdInitSend( ts_cmd_t *p_cmd, es_out_ts_id_t *p_es, block_t *p_block )
{
    p_cmd->i_type = C_SEND;
    p_cmd->i_date = mdate();
...
static int CmdInitDel( ts_cmd_t *p_cmd, es_out_ts_id_t *p_es )
{
    p_cmd->i_type = C_DEL;
    p_cmd->i_date = mdate();
...
static int CmdInitControl( ts_cmd_t *p_cmd, int i_query, va_list args, bool b_copy )
{
    ...
    /* Modified control */
    case ES_OUT_SET_ES:  /* arg1= es_out_ts_id_t*                   */
    case ES_OUT_SET_ES2: // Added by K.C.Y
    case ES_OUT_RESTART_ES:  /* arg1= es_out_ts_id_t*                   */
    case ES_OUT_SET_ES_DEFAULT: /* arg1= es_out_ts_id_t*                */
        p_cmd->u.control.u.p_es = (es_out_ts_id_t*)va_arg( args, es_out_ts_id_t * );
        break;
    ...
    case ES_OUT_SET_ES_STATE:/* arg1= es_out_ts_id_t* arg2=bool   */
        p_cmd->u.control.u.es_bool.p_es = (es_out_ts_id_t*)va_arg( args, es_out_ts_id_t * );
        p_cmd->u.control.u.es_bool.b_bool = (bool)va_arg( args, int );
        break;

    case ES_OUT_SET_ES_FMT:     /* arg1= es_out_ts_id_t* arg2=es_format_t* */
    {
        p_cmd->u.control.u.es_fmt.p_es = (es_out_ts_id_t*)va_arg( args, es_out_ts_id_t * );
...
{
    if( dirname != NULL
     && asprintf( filename, "%s"DIR_SEP "package-timeshift.XXXXXX",
                  dirname ) >= 0 )
    {
    ...
    *filename = strdup( DIR_SEP"tmp"DIR_SEP "package-timeshift.XXXXXX" );
