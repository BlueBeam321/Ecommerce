    ...
    if( i_read > 0 )
    {
        if( priv->text.char_width > 1 )
        {
            ...
            if( vlc_iconv( priv->text.conv, &p_in, &i_in, &p_out, &i_out ) == (size_t)-1 )
            {
                ...
                msg_Dbg( s, "original: %d, in %d, out %d", i_line, i_in, i_out );
            }
            free( p_line );
            p_line = psz_new_line;
...
static ssize_t vlc_stream_CopyBlock(block_t ** pp,
                                    void *buf, size_t len)
{
    block_t *block = *pp;
...
static ssize_t vlc_stream_ReadRaw(stream_t *s, void *buf, size_t len)
{
    ...
    if (s->pf_read != NULL)
    {
        ...
        if (buf == NULL)
        {
            ...
            ret = s->pf_read(s, dummy, len/*sizeof (dummy)*/); // K.C.Y
            free(dummy);
        }
        else
    ...
    if (s->pf_block != NULL)
    {
        ...
        ret = vlc_stream_CopyBlock(&priv->block, buf, len);
        //if (eof)
        //    msg_Dbg(NULL, "KCY: block=%p, len=%d, ret=%d", priv->block, len, ret);
        if (ret >= 0)
            return ret;
        return eof ? 0 : -1;
    }
