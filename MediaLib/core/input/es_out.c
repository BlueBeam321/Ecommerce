...
#include "common/core/stream_output/stream_output.h"
...
typedef struct
{
    ...
    es_out_id_t *p_sub_es; /* Added by K.C.Y */
...
static void         EsOutSelect(es_out_t *, es_out_id_t *es, bool b_force, bool b_sub); // Modified by K.C.Y
    ...
    const char *psz_debug)
{
    ...
    p_props->p_sub_es = NULL; // Added by K.C.Y
    ...
    if (!input_priv(p_input)->b_preparsing && psz_langvar)
    {
        ...
        {
            for (int i = 0; p_props->ppsz_language[i]; i++)
                msg_Dbg(p_input, "selected %s language[%d] %s", psz_debug, i, p_props->ppsz_language[i]);
        }
        free(psz_string);
    }
}
...
//static es_out_id_t es_cat[DATA_ES]; // Commented by K.C.Y

static es_out_id_t *EsOutGetFromID(es_out_t *out, int i_id)
{
    es_out_sys_t* p_sys = (es_out_sys_t*)out->p_sys;
    //if (i_id < 0) // Commented by K.C.Y
    //{
...
static void EsOutDecodersStopBuffering(es_out_t *out, bool b_forced)
{
    ...
    mtime_t i_stream_duration;
    mtime_t i_system_duration;
    if (input_clock_GetState(p_sys->p_pgrm->p_clock, &i_stream_start, &i_system_start, &i_stream_duration, &i_system_duration))
        return;
...
static void EsOutESVarUpdateGeneric(es_out_t *out, int i_id,
     es_format_t *fmt, const char *psz_language,
    bool b_delete)
{
    es_out_sys_t* p_sys = (es_out_sys_t*)out->p_sys;
    input_thread_t    *p_input = p_sys->p_input;
    ...
    if (fmt->i_cat == AUDIO_ES)
    {
        ...
        if (!fmt->psz_description)
            fmt->psz_description = strdup("Unkown Track");
        if (!fmt->psz_language)
            fmt->psz_language = strdup("Unkown Language");
    }
    ...
    if (fmt->psz_description && *fmt->psz_description)
    {
        ...
        {
            if (asprintf(&text.psz_string, "%s - [%s]", fmt->psz_description, psz_language) == -1)
                text.psz_string = NULL;
        }
        else
            text.psz_string = strdup(fmt->psz_description);
    }
    else
    {
        if (psz_language && *psz_language)
...
static void EsOutProgramSelect(es_out_t *out, es_out_pgrm_t *p_pgrm)
{
    ...
    if (p_sys->p_pgrm)
    {
        ...
        {
            if (p_sys->es[i]->p_pgrm == old && EsIsSelected(p_sys->es[i]) && p_sys->i_mode != ES_OUT_MODE_ALL)
        ...
        /* Added by K.C.Y */
        p_sys->audio.p_sub_es = NULL;
        p_sys->video.p_sub_es = NULL;
        p_sys->sub.p_sub_es = NULL;
    }
    ...
    for (i = 0; i < p_sys->i_es; i++)
    {
        ...
        EsOutSelect(out, p_sys->es[i], false, false); // Modified by K.C.Y
    }
    ...
    if (p_pgrm->p_meta)
    {
        input_item_SetESNowPlaying(input_priv(p_input)->p_item, vlc_meta_Get(p_pgrm->p_meta, vlc_meta_ESNowPlaying));
        input_item_SetPublisher(input_priv(p_input)->p_item, vlc_meta_Get(p_pgrm->p_meta, vlc_meta_Publisher));
        input_item_SetTitle(input_priv(p_input)->p_item, vlc_meta_Get(p_pgrm->p_meta, vlc_meta_Title));
...
static es_out_pgrm_t *EsOutProgramAdd(es_out_t *out, int i_group)
{
    ...
    /*if (EsOutIsProgramVisible(out, i_group)) // Commented by K.C.Y
...
{
    es_out_sys_t* p_sys = (es_out_sys_t*)p_out->p_sys;
...
static void EsOutProgramMeta(es_out_t *out, int i_group, const vlc_meta_t *p_meta)
{
    ...
    if (psz_title && *psz_title)
    {
        ...
        {
#if 0 //by RMS, Reason:Remove Provider
            if (asprintf( &psz_text, "%s [%s]", psz_title, psz_provider ) < 0)
                psz_text = NULL;
#else
            if (asprintf(&psz_text, "%s", psz_title) < 0)
                psz_text = NULL;
#endif
        }
        else
    ...
    {
        if (p_cat)
            info_category_AddInfo(p_cat, vlc_gettext(ppsz_all_keys[i]), "%s", vlc_meta_GetExtra(p_meta, ppsz_all_keys[i]));
    ...
    if (psz_provider)
    {
        ...
        if (p_cat)
            info_category_AddInfo(p_cat, vlc_meta_TypeToLocalizedString(vlc_meta_Publisher), "%s", psz_provider);
    }
    if (p_cat)
...
{
    es_out_sys_t* p_sys = (es_out_sys_t*)p_out->p_sys;
    EsOutMeta(p_out, p_meta, (p_sys->p_pgrm && p_sys->p_pgrm->p_meta) ? p_sys->p_pgrm->p_meta : NULL);
}
...
static es_out_id_t *EsOutAddSlave(es_out_t *out, const es_format_t *fmt, es_out_id_t *p_master)
{
    ...
    case VIDEO_ES:
        ...
        if (es->fmt.video.i_frame_rate && es->fmt.video.i_frame_rate_base)
            vlc_ureduce(&es->fmt.video.i_frame_rate, &es->fmt.video.i_frame_rate_base, es->fmt.video.i_frame_rate, es->fmt.video.i_frame_rate_base, 0);
        break;
    ...
    EsOutSelect(out, es, false, false); // Modified by K.C.Y
...
static bool EsIsSelected(es_out_id_t *es)
{
    if (es->p_master)
    {
        ...
        if (es->p_master->p_dec)
        {
            ...
            input_DecoderGetCcState(es->p_master->p_dec, es->fmt.i_codec, i_channel, &b_decode);
        }
        return b_decode;
    }
    else
...
static void EsSelect(es_out_t *out, es_out_id_t *es)
{
    ...
    if (es->p_master)
    {
        ...
        if (i_channel == -1 ||
            input_DecoderSetCcState(es->p_master->p_dec, es->fmt.i_codec, i_channel, true))
            return;
    }
    else
    {
        ...
        if (es->fmt.i_cat == SPU_ES)
        {
            ...
            {
                msg_Dbg(p_input, "spu is disabled, not selecting ES 0x%x", es->i_id);
                return;
            }
        }
...
static void EsUnselect(es_out_t *out, es_out_id_t *es, bool b_update)
{
    ...
    if (es->p_master)
    {
        if (es->p_master->p_dec)
        {
            int i_channel = EsOutGetClosedCaptionsChannel(&es->fmt);
            if (i_channel != -1)
                input_DecoderSetCcState(es->p_master->p_dec, es->fmt.i_codec, i_channel, false);
        }
    }
    else
    {
 ...
 * \return nothing
 */
static void EsOutSelect(es_out_t *out, es_out_id_t *es, bool b_force, bool b_sub) // Modified by K.C.Y
{
    es_out_sys_t* p_sys = (es_out_sys_t*)out->p_sys;
    ...
    bool b_auto_unselect = p_esprops && p_sys->i_mode == ES_OUT_MODE_AUTO &&
        ...
        ((p_esprops->p_main_es && p_esprops->p_main_es != es) || (p_esprops->p_sub_es && p_esprops->p_sub_es != es));
    ...
    if (p_sys->i_mode == ES_OUT_MODE_ALL || b_force)
    {
        if (!EsIsSelected(es))
        {
            if (b_auto_unselect)
            {
                ...
                if (b_sub)
                {
                    if (p_esprops->p_sub_es)
                        EsUnselect(out, p_esprops->p_sub_es, false);
                }
                else
                {
                    if (p_esprops->p_main_es)
    ...
    else if (p_sys->i_mode == ES_OUT_MODE_PARTIAL)
    {
        ...
        if (prgms != NULL)
        {
            ...
            for (const char *prgm = strtok_r(prgms, ",", &buf); prgm != NULL; prgm = strtok_r(NULL, ",", &buf))
            {
    ...
    else if (p_sys->i_mode == ES_OUT_MODE_AUTO)
    {
        ...
        else if (p_esprops->ppsz_language)
        {
            ...
            int current_es_idx = (p_esprops->p_main_es == NULL) ? -1 :
                LanguageArrayIndex(p_esprops->ppsz_language, p_esprops->p_main_es->psz_language_code);
            int es_idx = LanguageArrayIndex(p_esprops->ppsz_language, es->psz_language_code);
            ...
            else if (i_stop_idx < 0) /* If not fallback disabled by 'none' */
            {
                ...
                if (current_es_idx < 0) /* No es is currently selected by lang pref */
                {
                    ...
                    else if (p_esprops->p_main_es == NULL || es->fmt.i_priority > p_esprops->p_main_es->fmt.i_priority)
                    {
    ...
    for (int i = 0; i_bitmap > 0; i++, i_bitmap >>= 1, i_existingbitmap >>= 1)
    {
        ...
        *pp_es = EsOutAddSlave(out, &fmt, parent);
        ...
        if (p_sys->sub.i_channel == i)
            EsOutSelect(out, *pp_es, true, false); // Modified by K.C.Y
    }
}
...
static int EsOutSend(es_out_t *out, es_out_id_t *es, block_t *p_block)
{
    ...
    if (libvlc_stats(p_input))
    {
        ...
        stats_Update(input_priv(p_input)->counters.p_demux_read, p_block->i_buffer, &i_total);
    ...
    if (es->p_dec_record)
    {
        ...
        if (p_dup)
            input_DecoderDecode(es->p_dec_record, p_dup, input_priv(p_input)->b_out_pace_control);
    }
    input_DecoderDecode(es->p_dec, p_block, input_priv(p_input)->b_out_pace_control);
    ...
    if (var_InheritInteger(p_input, "captions") == 708)
        EsOutCreateCCChannels(out, VLC_CODEC_CEA708, desc.i_708_channels, _("DTVCC Closed captions %u"), es);
    EsOutCreateCCChannels(out, VLC_CODEC_CEA608, desc.i_608_channels, _("Closed captions %u"), es);
...
static void EsOutDel(es_out_t *out, es_out_id_t *es)
{
    ...
    if (p_esprops)
    {
        ...
        else if (p_esprops->p_sub_es == es) // Added by K.C.Y
            p_esprops->p_sub_es = NULL;
        p_esprops->i_count--;
    }
...
static int EsOutControlLocked(es_out_t *out, int i_query, va_list args)
{
    ...
    case ES_OUT_SET_ES:
    case ES_OUT_SET_ES2: // Added by K.C.Y
    case ES_OUT_RESTART_ES:
    {
//#define IGNORE_ES DATA_ES
        //es_out_id_t *es = va_arg(args, es_out_id_t *); // Commented by K.C.Y
        int i_id = va_arg(args, int); // Added by K.C.Y

        /*enum es_format_category_e i_cat; // Commented by K.C.Y
        if (es == NULL)
        i_cat = UNKNOWN_ES;
        ...
        i_cat = IGNORE_ES;*/
        if (i_id < 0) // Added by K.C.Y
        {
            es_out_es_props_t* p_esprops = GetPropsByCat(p_sys, -i_id);
            if (i_query == ES_OUT_SET_ES)
            {
                if (p_esprops && p_esprops->p_main_es && EsIsSelected(p_esprops->p_main_es))
                {
                    EsUnselect(out, p_esprops->p_main_es, p_esprops->p_main_es->p_pgrm == p_sys->p_pgrm);
                    p_esprops->p_main_es = NULL;
                }
            }
            else if (i_query == ES_OUT_SET_ES2)
            {
                if (p_esprops && p_esprops->p_sub_es && EsIsSelected(p_esprops->p_sub_es))
                {
                    EsUnselect(out, p_esprops->p_sub_es, p_esprops->p_sub_es->p_pgrm == p_sys->p_pgrm);
                    p_esprops->p_sub_es = NULL;
                }
            }
            else
            {
                if (p_esprops && ((p_esprops->p_main_es && p_esprops->p_main_es->p_dec) || (p_esprops->p_sub_es && p_esprops->p_sub_es->p_dec)))
                {
                    if (p_esprops->p_main_es)
                    {
                        EsDestroyDecoder(out, p_esprops->p_main_es);
                        EsCreateDecoder(out, p_esprops->p_main_es);
                    }

                    if (p_esprops->p_sub_es)
                    {
                        EsDestroyDecoder(out, p_esprops->p_sub_es);
                        EsCreateDecoder(out, p_esprops->p_sub_es);
                    }
                }
            }
            break;
        ...
        for (int i = 0; i < p_sys->i_es; i++)
        {
            if (i_id == p_sys->es[i]->i_id) // Added by K.C.Y
            {
                if (i_query == ES_OUT_RESTART_ES && p_sys->es[i]->p_dec)
                {
                    ...
                    EsOutSelect(out, p_sys->es[i], true, false);
                else if (i_query == ES_OUT_SET_ES2)
                    EsOutSelect(out, p_sys->es[i], true, true);
                break;
            }
            //if (i_cat == IGNORE_ES) // Commented by K.C.Y
            //{
            //    if (es == p_sys->es[i])
            //    {
            //        if (i_query == ES_OUT_RESTART_ES && p_sys->es[i]->p_dec)
        ...
        else if (p_pgrm == p_sys->p_pgrm)
        {
                ...
                !input_priv(p_sys->p_input)->b_out_pace_control))
            {
                ...
                else
                {
                    ...
                    /* Force a rebufferrization when we are too late */

                    /* It is not really good, as we throw away already buffered data
                     * TODO have a mean to correctly reenter bufferrization */
    ...
    case ES_OUT_SET_ES_BY_ID2: // Added by K.C.Y
    ...
    case ES_OUT_SET_ES_DEFAULT_BY_ID:
    {
        ...
        int i_ret = es_out_Control(out, i_new_query, i_id/*p_es*/); // Modified by K.C.Y
    ...
    case ES_OUT_SET_JITTER:
    {
        ...
        for (int i = 0; i < p_sys->i_pgrm && b_change_clock; i++)
            input_clock_SetJitter(p_sys->pgrm[i]->p_clock, i_pts_delay + i_pts_jitter, i_cr_average);
        return VLC_SUCCESS;
    }
    ...
    default:
        msg_Err(p_sys->p_input, "unknown query 0x%x in %s", i_query, __FUNCTION__);
        return VLC_EGENERIC;
    }
}
...
static void EsOutUpdateInfo(es_out_t *out, es_out_id_t *es, const es_format_t *fmt, const vlc_meta_t *p_meta)
{
    ...
    if (es->i_meta_id != es->i_id)
        info_category_AddInfo(p_cat, _("Original ID"), "%d", es->i_id);

    const vlc_fourcc_t i_codec_fourcc = (p_fmt_es->i_original_fourcc) ?
        p_fmt_es->i_original_fourcc : p_fmt_es->i_codec;
    const char *psz_codec_description = vlc_fourcc_GetDescription(p_fmt_es->i_cat, i_codec_fourcc);
    if (psz_codec_description && *psz_codec_description)
        info_category_AddInfo(p_cat, _("Codec"), "%s (%.4s)", psz_codec_description, (char*)&i_codec_fourcc);
    else if (i_codec_fourcc != VLC_FOURCC(0, 0, 0, 0))
        info_category_AddInfo(p_cat, _("Codec"), "%.4s", (char*)&i_codec_fourcc);

    if (es->psz_language && *es->psz_language)
        info_category_AddInfo(p_cat, _("Language"), "%s", es->psz_language);
    if (fmt->psz_description && *fmt->psz_description)
        info_category_AddInfo(p_cat, _("Description"), "%s", fmt->psz_description);
    ...
    case AUDIO_ES:
        ...
        if (fmt->audio.i_physical_channels)
            info_category_AddInfo(p_cat, _("Channels"), "%s", _(aout_FormatPrintChannels(&fmt->audio)));

        if (fmt->audio.i_rate != 0)
        {
            info_category_AddInfo(p_cat, _("Sample rate"), _("%u Hz"), fmt->audio.i_rate);
        ...
        if (i_bitspersample != 0)
            info_category_AddInfo(p_cat, _("Bits per sample"), "%u", i_bitspersample);

        if (fmt->i_bitrate != 0)
        {
            info_category_AddInfo(p_cat, _("Bitrate"), _("%u kb/s"), fmt->i_bitrate / 1000);
            /* FIXME that should be removed or improved ! (used by text/strings.c) */
        ...
        for (int i = 0; i < AUDIO_REPLAY_GAIN_MAX; i++)
        {
            ...
            info_category_AddInfo(p_cat, psz_name, _("%.2f dB"), p_rg->pf_gain[i]);
        }
        break;
    ...
    case VIDEO_ES:
        ...
        if (fmt->video.i_visible_width > 0 && fmt->video.i_visible_height > 0)
            info_category_AddInfo(p_cat, _("Video resolution"), "%ux%u", fmt->video.i_visible_width, fmt->video.i_visible_height);

        if (fmt->video.i_width > 0 && fmt->video.i_height > 0)
            info_category_AddInfo(p_cat, _("Buffer dimensions"), "%ux%u", fmt->video.i_width, fmt->video.i_height);

        if (fmt->video.i_frame_rate > 0 && fmt->video.i_frame_rate_base > 0)
       {
           div = lldiv((float)fmt->video.i_frame_rate /
               fmt->video.i_frame_rate_base * 1000000,
               1000000);
           if (div.rem > 0)
               info_category_AddInfo(p_cat, _("Frame rate"), "%"PRId64".%06u", div.quot, (unsigned int)div.rem);
           else
               info_category_AddInfo(p_cat, _("Frame rate"), "%"PRId64, div.quot);
       }
        if (fmt->i_codec != p_fmt_es->i_codec)
       {
           const char *psz_chroma_description = vlc_fourcc_GetDescription(VIDEO_ES, fmt->i_codec);
           if (psz_chroma_description)
               info_category_AddInfo(p_cat, _("Decoded format"), "%s", psz_chroma_description);
       }
       {
           ...
           info_category_AddInfo(p_cat, _("Orientation"), "%s", _(orient_names[fmt->video.orientation]));
       }
       if (fmt->video.primaries != COLOR_PRIMARIES_UNDEF)
       {
           static const char primaries_names[][32] = {
               [COLOR_PRIMARIES_UNDEF] = N_("Undefined"),
               [COLOR_PRIMARIES_BT601_525] = N_("ITU-R BT.601 (525 lines, 60 Hz)"),
               [COLOR_PRIMARIES_BT601_625] = N_("ITU-R BT.601 (625 lines, 50 Hz)"),
           ...
           info_category_AddInfo(p_cat, _("Color primaries"), "%s", _(primaries_names[fmt->video.primaries]));
       }
       if (fmt->video.transfer != TRANSFER_FUNC_UNDEF)
       {
           ...
           info_category_AddInfo(p_cat, _("Color transfer function"), "%s", _(func_names[fmt->video.transfer]));
       }
       ...
       if (fmt->video.chroma_location != CHROMA_LOCATION_UNDEF)
       {
           ...
           info_category_AddInfo(p_cat, _("Chroma location"), "%s", _(c_loc_names[fmt->video.chroma_location]));
       }
    ...
    if (p_meta)
    {
        ...
        for (int i = 0; ppsz_all_keys && ppsz_all_keys[i]; i++)
        {
            ...
            if (psz_value)
                info_category_AddInfo(p_cat, vlc_gettext(psz_key), "%s", vlc_gettext(psz_value));
            free(psz_key);
        }
        free(ppsz_all_keys);
    }
...
static void EsDeleteInfo(es_out_t *out, es_out_id_t *es)
{
    ...
    {
        input_Control(p_sys->p_input, INPUT_DEL_INFO, psz_info_category, NULL);
