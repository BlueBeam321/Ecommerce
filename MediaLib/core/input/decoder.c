...
#include "decoder.h"
#include "event.h"
#include "common/core/input/resource.h"
#include "common/core/video_output/vout_control.h"
 ...
 */
static int LoadDecoder(decoder_t *p_dec, bool b_packetizer, const es_format_t * p_fmt)
{
...
static int ReloadDecoder(decoder_t *p_dec, bool b_packetizer, const es_format_t * p_fmt, enum reload reload)
{
...
static int aout_update_format(decoder_t *p_dec)
{
    ...
    if (aout_replaygain_changed(&p_dec->fmt_in.audio_replay_gain, &p_owner->fmt.audio_replay_gain))
    {
...
static int vout_update_format(decoder_t *p_dec)
{
    ...
    if (memcmp(&p_dec->fmt_out.video.mastering, &p_owner->fmt.video.mastering, sizeof(p_owner->fmt.video.mastering)) ||
        p_dec->fmt_out.video.lighting.MaxCLL != p_owner->fmt.video.lighting.MaxCLL ||
        p_dec->fmt_out.video.lighting.MaxFALL != p_owner->fmt.video.lighting.MaxFALL)
    {
...
static void RequestReload(decoder_t * p_dec)
{
    ...
    atomic_int expected = RELOAD_NO_REQUEST;
...
static int DecoderTimedWait(decoder_t *p_dec, mtime_t deadline)
{
    ...
    while (!p_owner->flushing
        && vlc_fifo_TimedWaitCond(p_owner->p_fifo, &p_owner->wait_timed, deadline) == 0);
...
static void DecoderProcessSout(decoder_t *p_dec, block_t *p_block)
{
    ...
    while ((p_sout_block = p_dec->pf_packetize(p_dec, pp_block)))
    {
        if (p_owner->p_sout_input == NULL)
        {
            ...
            if (p_dec->fmt_in.psz_language)
            {
                ...
                p_owner->fmt.psz_language = strdup(p_dec->fmt_in.psz_language);
            }
            vlc_mutex_unlock(&p_owner->lock);

            p_owner->p_sout_input = sout_InputNew(p_owner->p_sout, &p_owner->fmt);
...
static void DecoderPlayCc(decoder_t *p_dec, block_t *p_cc, const decoder_cc_desc_t *p_desc)
{
    ...
    /* Fanout data to all decoders. We do not know if es_out
       selected 608 or 708. */
    uint64_t i_bitmap = p_owner->cc.desc.i_608_channels | p_owner->cc.desc.i_708_channels;
...
static int DecoderPlayVideo(decoder_t *p_dec, picture_t *p_picture, unsigned * pi_lost_sum)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    vout_thread_t  *p_vout = p_owner->p_vout;
    bool prerolled;

    //msg_Dbg(NULL, "KCY: i_pts=%lld, i_dts=%lld", p_picture->date, p_picture->date);
...
static int DecoderQueueVideo(decoder_t *p_dec, picture_t *p_pic)
{
    ...
    /*if (p_pic )
    {
        if (p_pic->date < p_owner->i_prev_date)
        {
            msg_Err(p_dec, "presentation timestamp is incorrect.");
            if (p_dec->fmt_in.video.i_frame_rate)
            {
                p_pic->date = p_owner->i_prev_date + CLOCK_FREQ * p_dec->fmt_in.video.i_frame_rate_base / p_dec->fmt_in.video.i_frame_rate;
                p_owner->i_prev_date = p_pic->date;
            }
        }
        p_owner->i_prev_date = p_pic->date;
    }*/
...
static int DecoderPlayAudio(decoder_t *p_dec, block_t *p_audio, unsigned * pi_lost_sum)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    bool prerolled;
...
static bool PacketizeBlock(decoder_t* p_dec, block_t** pp_block, block_t** pp_packetized_block) // Added by K.C.Y
{
    decoder_owner_sys_t* p_owner = p_dec->p_owner;
    ...
    /*if (pp_block && *pp_block == NULL)
    {
        ...
        size_t count = block_FifoCount(p_owner->p_parsed_fifo);
        if (count > 0)
        {
            p_block = block_FifoShow(p_owner->p_parsed_fifo);
            if (p_block->i_flags & BLOCK_FLAG_TYPE_B)
                p_block = block_FifoGet(p_owner->p_parsed_fifo);
            else
            {
                bool b_ip_found = false;
                p_block = p_block->p_next;
                while (p_block)
                {
                    if (p_block->i_flags & (BLOCK_FLAG_TYPE_I | BLOCK_FLAG_TYPE_P))
                    {
                        b_ip_found = true;
                        break;
                    }
        ...
        if (pp_packetized_block)
            *pp_packetized_block = p_block;
        return p_block != NULL;
    }
    else*/
    {
        decoder_t *p_packetizer = p_owner->p_packetizer;
        block_t* p_packetized_block = p_packetizer->pf_packetize(p_packetizer, pp_block);
        bool ret = p_packetized_block != NULL;

#if 1
        es_format_t* p_fmt = &p_dec->fmt_out;
        if (p_packetized_block && p_fmt->i_cat == VIDEO_ES)
        {
            mtime_t last_pts = p_packetized_block->i_pts;
            size_t count = block_FifoCount(p_owner->p_parsed_fifo);
            if (p_packetized_block->i_flags & (BLOCK_FLAG_TYPE_I | BLOCK_FLAG_TYPE_P))
            {
                block_t* p_block = count > 0 ? block_FifoShow(p_owner->p_parsed_fifo) : NULL;
                while (p_block)
                {
                    if (p_block->i_flags & (BLOCK_FLAG_TYPE_I | BLOCK_FLAG_TYPE_P))
                    {
                        if (p_block->i_pts == VLC_TS_INVALID)
                            p_block->i_pts = p_owner->i_last_pts + (CLOCK_FREQ * p_fmt->video.i_frame_rate_base / p_fmt->video.i_frame_rate);
                        break;
                    }
                    p_block = p_block->p_next;
                }
                p_block = count > 0 ? block_FifoGet(p_owner->p_parsed_fifo) : NULL;
                block_FifoPut(p_owner->p_parsed_fifo, p_packetized_block);
                p_packetized_block = p_block;
            }
            else
            {
                if (count > 0)
                {
                    block_t* p_block = block_FifoShow(p_owner->p_parsed_fifo);
                    if (p_block->i_flags & BLOCK_FLAG_TYPE_B)
                        p_block = block_FifoGet(p_owner->p_parsed_fifo);
                    else
                    {
            ...
            p_owner->i_last_pts = last_pts;
        }
#endif

        if (pp_packetized_block)
            *pp_packetized_block = p_packetized_block;
        return ret;
    }
}
...
static void DecoderDecode(decoder_t *p_dec, block_t *p_block)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
   /* static int count = 0;
    static mtime_t prev_pts = 0;

    if (p_block && p_dec->fmt_in.i_cat == VIDEO_ES)
    {
        msg_Dbg(NULL, "RMS:DecoderInTime %04d pts=%lld, dts=%lld, dt=%lld", count++, p_block->i_pts, p_block->i_dts, p_block->i_dts - prev_pts);
        prev_pts = p_block->i_dts;
    }
    */
...
static void DecoderProcess(decoder_t *p_dec, block_t *p_block)
{
    ...
    atomic_int rel = atomic_exchange(&p_owner->reload, RELOAD_NO_REQUEST);
    if (rel)
    {
        msg_Warn(p_dec, "Reloading the decoder module%s",
            rel == RELOAD_DECODER_AOUT ? " and the audio output" : "");

        if (ReloadDecoder(p_dec, false, &p_dec->fmt_in, rel) != VLC_SUCCESS)
            goto error;
    }
    ...
    if (packetize)
    {
        ...
        while (PacketizeBlock(p_dec, pp_block, &p_packetized_block)/*(p_packetized_block = p_packetizer->pf_packetize(p_packetizer, pp_block))*/) // Modified by K.C.Y
        {
            if (p_packetized_block) // Modified by K.C.Y
            {
                /*if (p_dec->fmt_in.i_cat == VIDEO_ES)
                {
                    msg_Dbg(NULL, "KCY: i_pts=%lld, i_dts=%lld, type=%c",
                        p_packetized_block->i_pts, p_packetized_block->i_dts,
                        p_packetized_block->i_flags & BLOCK_FLAG_TYPE_I ? 'I' :
                        p_packetized_block->i_flags & BLOCK_FLAG_TYPE_P ? 'P' :
                        p_packetized_block->i_flags & BLOCK_FLAG_TYPE_B ? 'B' : 'X');
                }*/

                if (!es_format_IsSimilar(&p_dec->fmt_in, &p_packetizer->fmt_out))
                {
                    msg_Dbg(p_dec, "restarting module due to input format change (%s)",
                        p_dec->fmt_in.i_cat == VIDEO_ES ? "video" :
                        p_dec->fmt_in.i_cat == AUDIO_ES ? "audio" :
                        p_dec->fmt_in.i_cat == SPU_ES ? "spu" : "unknown");

                    /* Drain the decoder module */
                    DecoderDecode(p_dec, NULL);

                    if (ReloadDecoder(p_dec, false, &p_packetizer->fmt_out, RELOAD_DECODER) != VLC_SUCCESS)
                    {
    ...
    {
        /*if (p_dec->fmt_in.i_cat == VIDEO_ES && p_block)
        {
            msg_Dbg(NULL, "KCY: i_pts=%lld, i_dts=%lld, type=%c",
                p_block->i_pts, p_block->i_dts,
                p_block->i_flags & BLOCK_FLAG_TYPE_I ? 'I' :
                p_block->i_flags & BLOCK_FLAG_TYPE_P ? 'P' :
                p_block->i_flags & BLOCK_FLAG_TYPE_B ? 'B' : 'X');
        }*/
...
static void DecoderProcessFlush( decoder_t *p_dec )
{
    ...
    if (p_owner->p_parsed_fifo) // Added by K.C.Y
        block_FifoEmpty(p_owner->p_parsed_fifo);
    ...
    sout_instance_t *p_sout)
{
    ...
    p_owner->p_parsed_fifo = block_FifoNew(); // Added by K.C.Y
    p_owner->i_last_pts = VLC_TS_INVALID;
...
static void DeleteDecoder(decoder_t * p_dec)
{
    ...
    if (p_owner->p_parsed_fifo) // Added by K.C.Y
        block_FifoRelease(p_owner->p_parsed_fifo);
    ...
    sout_instance_t *p_sout)
{
    ...
    if (p_dec->fmt_out.i_cat == AUDIO_ES)
    {
        ...
        strcpy(thread_name, "audio_decoder");
    }
    else
    {
        i_priority = VLC_THREAD_PRIORITY_VIDEO;
        if (p_dec->fmt_out.i_cat == VIDEO_ES)
            strcpy(thread_name, "video_decoder");
        else
            strcpy(thread_name, "spu_decoder");
    }

    /* Spawn the decoder thread */
    if (vlc_clone(&p_dec->p_owner->thread, DecoderThread, p_dec, thread_name, i_priority))
    {
...
decoder_t *input_DecoderNew(input_thread_t *p_input,
    ...
    sout_instance_t *p_sout)
{
    return decoder_New(VLC_OBJECT(p_input), p_input, fmt, p_clock, input_priv(p_input)->p_resource, p_sout);
}
