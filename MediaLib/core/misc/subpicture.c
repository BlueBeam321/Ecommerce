...
subpicture_t *subpicture_NewFromPicture(vlc_object_t *p_obj, picture_t *p_picture, vlc_fourcc_t i_chroma)
{
    ...
    fmt_out.i_sar_num = fmt_out.i_sar_den = 0;
...
unsigned picture_BlendSubpicture(picture_t *dst, filter_t *blend, subpicture_t *src)
{
    ...
    for (subpicture_region_t *r = src->p_region; r != NULL; r = r->p_next)
    {
        ...
        if (filter_ConfigureBlend(blend, dst->format.i_width, dst->format.i_height, &r->fmt) ||
            filter_Blend(blend, dst, r->i_x, r->i_y, r->p_picture, src->i_alpha * r->i_alpha / 255))
        {
