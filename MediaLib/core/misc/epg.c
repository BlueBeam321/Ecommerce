...
static char* vlc_epg_string_Dup(char*str)
{
    int len = 0;
    uint16_t *p = str;
    uint16_t *q;
...
#else
        if (p_src->psz_description)
            p_evt->psz_description = vlc_epg_string_Dup(p_src->psz_description);
        if (p_src->psz_name)
            p_evt->psz_name = vlc_epg_string_Dup(p_src->psz_name);
        if (p_src->psz_short_description)
            p_evt->psz_short_description = vlc_epg_string_Dup(p_src->psz_short_description);
#endif
