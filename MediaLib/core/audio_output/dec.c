    ...
    const aout_request_vout_t *p_request_vout)
{
    ...
    owner->filters = aout_FiltersNew(p_aout, p_format,
        &owner->mixer_format,
        &owner->request_vout,
        &owner->filters_cfg);
...
static void aout_DecSilence(audio_output_t *aout, mtime_t length, mtime_t pts)
{
    ...
    block_t *block = block_Alloc(frames * fmt->i_bytes_per_frame / fmt->i_frame_length);
    if (unlikely(block == NULL))
        return; /* uho! */

    msg_Dbg(aout, "inserting %d zeroes", frames);
...
static void aout_DecSynchronize(audio_output_t *aout, mtime_t dec_pts, int input_rate)
{
     ...
     * completely. */
    if (drift > (owner->sync.discontinuity ? 0 : +3 * input_rate * AOUT_MAX_PTS_DELAY / INPUT_RATE_DEFAULT))
    {
        if (!owner->sync.discontinuity)
            msg_Warn(aout, "playback way too late (%"PRId64"): flushing buffers", drift);
        else
            msg_Dbg(aout, "playback too late (%"PRId64"): flushing buffers", drift);
    ...
    if (drift < (owner->sync.discontinuity ? 0 : -3 * input_rate * AOUT_MAX_PTS_ADVANCE / INPUT_RATE_DEFAULT))
    {
        if (!owner->sync.discontinuity)
            msg_Warn(aout, "playback way too early (%"PRId64"): playing silence", drift);
    ...
    /* Resampling */
    if (drift > +AOUT_MAX_PTS_DELAY && owner->sync.resamp_type != AOUT_RESAMPLING_UP)
    {
        msg_Warn(aout, "playback too late (%"PRId64"): up-sampling", drift);
        owner->sync.resamp_type = AOUT_RESAMPLING_UP;
        owner->sync.resamp_start_drift = +drift;
    }
    if (drift < -AOUT_MAX_PTS_ADVANCE && owner->sync.resamp_type != AOUT_RESAMPLING_DOWN)
    {
        msg_Warn(aout, "playback too early (%"PRId64"): down-sampling", drift);
...
int aout_DecPlay(audio_output_t *aout, block_t *block, int input_rate)
{
    ...
    block->i_length = CLOCK_FREQ * block->i_nb_samples / owner->input_format.i_rate;
    ...
    atomic_bool tmp_ret = atomic_exchange(&owner->vp.update, false);
    if (tmp_ret)
    {
...
void aout_DecGetResetStats(audio_output_t *aout, unsigned * lost, unsigned * played)
{
    aout_owner_t *owner = aout_owner(aout);
