    ...
    switch (propid)
    {
        case VLC_MODULE_CREATE:
        {
            ...
            *(va_arg (ap, module_t **)) = submodule;
            ...
            /* Inheritance. Ugly!! */
            submodule->pp_shortcuts = malloc(sizeof(*submodule->pp_shortcuts));
        ...
        {
            if (IsConfigIntegerType(item->i_type) || !CONFIG_ITEM(item->i_type))
            {
                item->orig.i =
                item->value.i = va_arg (ap, int64_t);
            }
            else if (IsConfigFloatType (item->i_type))
            {
                item->orig.f =
                item->value.f = va_arg (ap, double);
            }
            else if (IsConfigStringType (item->i_type))
            {
                const char *value = va_arg (ap, const char *);
        ...
        case VLC_CONFIG_LIST:
        {
            ...
            else if (IsConfigStringType (item->i_type))
            {
                ...
                const char **dst = malloc(sizeof(const char *) * len);
            ...
            const char **dtext = malloc(sizeof(const char *) * (len + 1));
