...
static ssize_t config_ListModules (const char *cap, char *** values,
                                   char *** texts)
{
    module_t **list;
    ...
    {
        vals[i + 1] = strdup(module_get_object(list[i]));
        txts[i + 1] = strdup(module_gettext(list[i], module_get_name(list[i], true)));
    }
...
int config_SortConfig (void)
{
    ...
    for (p = vlc_plugins; p != NULL; p = p->next)
    {
        ...
        for (item = p->conf.items, end = item + p->conf.size; item < end; item++)
        {
