...
VLC_USED static inline block_t *block_BytestreamPop(block_bytestream_t *p_bytestream)
{
    block_t *p_block;
...
static inline int block_PeekBytes(block_bytestream_t *p_bytestream, uint8_t *p_data, size_t i_data)
{
    if( block_BytestreamRemaining( p_bytestream ) < i_data )
        return VLC_EGENERIC;
    ...
    for (block_t *p_block = p_bytestream->p_block; p_block != NULL; p_block = p_block->p_next)
    {
...
static inline int block_GetBytes(block_bytestream_t *p_bytestream, uint8_t *p_data, size_t i_data)
{
    if (block_BytestreamRemaining(p_bytestream) < i_data)
        return VLC_EGENERIC;
    ...
    size_t i_copy = 0;
    block_t *p_block;
    for (p_block = p_bytestream->p_block; p_block != NULL; p_block = p_block->p_next)
    {
...
static inline int block_FindStartcodeFromOffset(block_bytestream_t *p_bytestream,
    size_t *pi_offset,
    ...
    *pi_offset -= i_size;
    ...
    for (; p_block != NULL; p_block = p_block->p_next)
    {
        for (i_offset = i_size; i_offset < p_block->i_buffer; i_offset++)
        {
            ...
            {
                const uint8_t *p_res = p_startcode_helper(&p_block->p_buffer[i_offset], &p_block->p_buffer[p_block->i_buffer]);
                if (p_res)
                {
