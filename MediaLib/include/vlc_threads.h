...
static inline int vlc_poll(struct pollfd *fds, unsigned nfds, int timeout)
{
    ...
    do {
        int ugly_timeout = (timeout >= 50 || timeout < 0) ? 50 : timeout;
        if (timeout >= 0)
 ...
 */
VLC_API int vlc_clone(vlc_thread_t *thread, void *(*entry)(void *), void *data, char* thread_name, int priority) VLC_USED;
...
void vlc_threads_setup(vlc_object_t *);
