...
libvlc_media_player_new( libvlc_instance_t *instance )
{
    ...
    var_Create(mp, "scramble", VLC_VAR_BOOL); //by RMS
    var_Create(mp, "vod-title", VLC_VAR_INTEGER); //by RMS
...
int libvlc_media_player_play(libvlc_media_player_t *p_mi)
{
    ...
    p_input_thread = input_Create(p_mi, p_mi->p_md->p_input_item, NULL, p_mi->input.p_resource, p_mi->input.p_renderer);
    unlock(p_mi);
    if (!p_input_thread)
    {
        unlock_input(p_mi);
...
LIBVLC_API int libvlc_media_player_change_program(libvlc_media_player_t *p_mi, int program) 
{
    input_thread_t * p_input = 0;
    p_input = libvlc_get_input_thread(p_mi);
    if (p_input)
    {
...
LIBVLC_API int libvlc_media_player_get_program_count(libvlc_media_player_t *p_mi)
{
    input_thread_t *p_input;
    vlc_value_t count_value;

    lock(p_mi);
    p_input = libvlc_get_input_thread(p_mi);
    if (p_input)
    {
        var_Change(p_input, "program", VLC_VAR_CHOICESCOUNT, &count_value, NULL);
        vlc_object_release(p_input);
    }
    unlock(p_mi);

    return  count_value.i_int;
}

LIBVLC_API int libvlc_media_player_get_program_info(libvlc_media_player_t *p_mi, int index, int *p_number, char *p_name)
{
    vlc_value_t number_list, name_list;
    input_thread_t *p_input;
    int ret = 0;
    int i_program_count;
    ...
    if (p_input)
    {
        ...
        if (index < number_list.p_list->i_count && number_list.p_list->i_count > 1)
        {
            *p_number = number_list.p_list->p_values[index].i_int;
            if (p_number)
                strcpy(p_name, name_list.p_list->p_values[index].psz_string);
        }
        else
            ret = -1;
...
LIBVLC_API int libvlc_media_player_get_epg_count(libvlc_media_player_t *p_mi, int number)
{
    input_thread_t *p_input = NULL;
    input_item_t *p_input_item = NULL;
    vlc_epg_t *p_epg_table = NULL;
    int i_epg_count;
    int i_event_count = 0;
    ...
    if (p_input)
    {
        ...
        i_epg_count = p_input_item->i_epg;
        for (int i = 0; i < i_epg_count; i++)
        {
            p_epg_table = p_input_item->pp_epg[i];
            if (p_epg_table->i_source_id == number)
            {
                i_event_count = p_epg_table->i_event;
                break;
            }
        }
        if (p_input_item)
    ...
    return  i_event_count;
}


LIBVLC_API int libvlc_media_player_get_epg_item(libvlc_media_player_t *p_mi, int number, int index, char *start, char *duration, char *style, char*title, char *conent)
{
    input_thread_t *p_input = NULL;
    input_item_t *p_input_item = NULL;
    vlc_epg_t *p_epg_table = NULL;
    vlc_epg_event_t *p_epg_event = NULL;
    int i_epg_count, i_event_count;
    int ret = 0;
    bool b_find = false;
    struct tm *t;
    ...
    if (p_input)
    {
        ...
        if (i_epg_count)
        {
            ...
            if (b_find && index < i_event_count)
            {
                p_epg_event = p_epg_table->pp_event[index];
                
                t = localtime(&p_epg_event->i_start);
                sprintf(start, "%04d/%02d/%02d %02d:%02d:%02d", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);
                sprintf(duration, "%02d:%02d", p_epg_event->i_duration / 60, p_epg_event->i_duration % 60);
                
                uint16_t *p, *q;

                p = p_epg_event->psz_name;
                q = style;
                while (*p)
                    *q++ = *p++;

                p = p_epg_event->psz_short_description;
                q = title;
                while (*p)
                    *q++ = *p++;

                p = p_epg_event->psz_description;
                q = conent;
                while (*p)
                    *q++ = *p++;
...
LIBVLC_API void libvlc_media_player_set_scramble(libvlc_media_player_t *p_mi, bool b_scramble)
{
    lock(p_mi);

    var_SetBool(p_mi, "scramble", b_scramble);

    unlock(p_mi);
}


LIBVLC_API void libvlc_media_player_set_vod_title_count(libvlc_media_player_t *p_mi, int i_title)
{
    lock(p_mi);

    var_SetInteger(p_mi, "vod-title", i_title);

    unlock(p_mi);
}

LIBVLC_API int libvlc_media_player_key_init()
{
    return 0;
}

LIBVLC_API int libvlc_media_player_get_machine_id(libvlc_media_player_t *p_mi, char* serial, char* machine_id)
{
    uint8_t *p = sinjong_generate_machine_id(serial);
    strcpy(machine_id, p);
    free(p);
    return 0;
}

LIBVLC_API int libvlc_media_player_set_lincense(libvlc_media_player_t *p_mi, char* public_key, char* lincense, char* machine_id)
{
    if (public_key)
        sinjong_set_public_key(public_key);
    return sinjong_check_license(lincense, machine_id);
}

LIBVLC_API int64_t libvlc_media_player_get_epg_time(libvlc_media_player_t *p_mi)
{
    input_thread_t *p_input = NULL;
    input_item_t *p_input_item = NULL;
    int64_t i_epg_time;
    ...
    if (p_input)
    {
        ...
        i_epg_time = p_input_item->i_epg_time;
