...
int libvlc_video_get_spu( libvlc_media_player_t *p_mi, bool b_sub )
{
    input_thread_t *p_input_thread = libvlc_get_input_thread( p_mi );
    ...
    int i_spu = var_GetInteger( p_input_thread, b_sub ? "spu-es-sub" : "spu-es" ); // Modified by K.C.Y
    vlc_object_release( p_input_thread );
    return i_spu;
}
...
int libvlc_video_set_spu(libvlc_media_player_t *p_mi, int i_spu, bool b_sub)
{
    input_thread_t *p_input_thread = libvlc_get_input_thread(p_mi);
    vlc_value_t list;
    int i_ret = -1;
    ...
    for (int i = 0; i < list.p_list->i_count; i++)
    {
        ...
        {
            if (var_SetInteger(p_input_thread, b_sub ? "spu-es-sub" : "spu-es", i_spu) < 0) // Modified by K.C.Y
                break;
            i_ret = 0;
            goto end;
        }
...
static bool find_sub_source_by_name( libvlc_media_player_t *p_mi, const char * name )
{
...
static void
set_value( libvlc_media_player_t *p_mi, const char * name,
           const opt_t * opt, unsigned i_expected_type,
...
static float
get_float( libvlc_media_player_t *p_mi, const char * name,
            const opt_t * opt )
{
    if( !opt ) return 0.0;
