...
#include <vlc_cpu.h>
#include "common/modules/packetizer/h264_nal.h"
...
#define MODULE_NAME     codec_omxil
#define MODULE_STRING   "codec_omxil"
...
static int HwBuffer_Start( decoder_t *p_dec, OmxPort *p_port )
{
    ...
    if (vlc_clone(&p_port->p_hwbuf->dequeue_thread, DequeueThread, p_dec, "codec_omxil_dequeue", VLC_THREAD_PRIORITY_LOW))
    {
        p_port->p_hwbuf->b_run = false;
        HWBUFFER_UNLOCK( p_port );
        return -1;
