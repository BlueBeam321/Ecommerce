...
#include "common/modules/codec/hxxx_helper.h"
#include "OMX_Core.h"
#include "OMX_Component.h"
#include "omxil_utils.h"
#include "common/modules/video_output/android/display.h"

#define MODULE_NAME     codec_mediacodec
#define MODULE_STRING   "codec_mediacodec"
...
vlc_module_begin ()
    ...
    add_submodule ()
        ...
        add_shortcut("mediacodec_it02_jni")
    add_submodule ()
        set_capability("audio decoder", 0)
        set_callbacks(OpenDecoderJni, CloseDecoder)
        add_shortcut("mediacodec_it02_jni")
...
static void HXXXInitSize(decoder_t *p_dec, bool *p_size_changed)
{
    if (p_size_changed)
    {
        ...
        *p_size_changed = (i_w != p_sys->video.i_input_width || i_h != p_sys->video.i_input_height);
...
static int HEVCSetCSD(decoder_t *p_dec, bool *p_size_changed)
{
    ...
    assert(hh->hevc.i_vps_count > 0 || hh->hevc.i_sps_count > 0 || hh->hevc.i_pps_count > 0 );
...
static int ParseVideoExtraHEVC(decoder_t *p_dec, uint8_t *p_extra, int i_extra)
{
    ...
    if (hh->hevc.i_vps_count > 0 || hh->hevc.i_sps_count > 0 || hh->hevc.i_pps_count > 0 )
...
static int StartMediaCodec(decoder_t *p_dec)
{
    ...
    {
        args.video.i_width = p_dec->fmt_out.video.i_visible_width; // i_width, Modified by K.C.Y
        args.video.i_height = p_dec->fmt_out.video.i_visible_height; // i_height, Modified by K.C.Y
...
static int OpenDecoder(vlc_object_t *p_this, pf_MediaCodecApi_init pf_init)
{
    ...
    msg_Dbg(p_dec, "KCY: Input Format(width=%d, height=%d, visible_width=%d, visible_height=%d)", 
    ...
    else
    {
        ...
        case VLC_CODEC_OPUS:
            ...
            break;
        case VLC_CODEC_ALAW:
            mime = "audio/g711-alaw";
            break;
        case VLC_CODEC_MULAW:
            mime = "audio/g711-mlaw";
            break;
        case VLC_CODEC_FLAC:
            mime = "audio/flac";
            break;
    ...
    if (p_dec->fmt_in.i_cat == VIDEO_ES)
    {
        ...
        case VLC_CODEC_H264:
        case VLC_CODEC_HEVC:
            hxxx_helper_init(&p_sys->video.hh, VLC_OBJECT(p_dec), p_dec->fmt_in.i_codec, false);
            break;
        }
        ...
        TAB_INIT(p_sys->video.i_inflight_pictures, p_sys->video.pp_inflight_pictures);
        ...
        if (var_InheritBool(p_dec, CFG_PREFIX "dr"))
        {
            ...
            p_sys->video.i_input_width = /*p_dec->fmt_out.video.i_visible_width =*/ p_dec->fmt_out.video.i_width; // Commented by K.C.Y
            p_sys->video.i_input_height = /*p_dec->fmt_out.video.i_visible_height =*/ p_dec->fmt_out.video.i_height; // Commented by K.C.Y
            if (p_dec->fmt_out.video.i_visible_width == 0 || p_dec->fmt_out.video.i_visible_height == 0) // Added block by K.C.Y
            {
    ...
    else
    {
        ...
        if ((p_sys->api.i_quirks & MC_API_AUDIO_QUIRKS_NEED_CHANNELS) && !p_sys->audio.i_channels)
        {
    ...
    if ((p_sys->api.i_quirks & MC_API_QUIRKS_NEED_CSD) && !p_sys->i_csd_count && !p_sys->b_adaptive)
    {
        switch (p_dec->fmt_in.i_codec)
        {
        case VLC_CODEC_H264:
        case VLC_CODEC_HEVC:
            msg_Warn(p_dec, "Not CSD found for %4.4s", (const char *) &p_dec->fmt_in.i_codec);
    ...
    if (vlc_clone(&p_sys->out_thread, OutThread, p_dec, "codec_mediacodec_output", VLC_THREAD_PRIORITY_LOW))
    {
...
static void CleanDecoder(decoder_t *p_dec)
{
    ...
    if (p_dec->fmt_in.i_cat == VIDEO_ES)
    {
        if (p_dec->fmt_in.i_codec == VLC_CODEC_H264 || p_dec->fmt_in.i_codec == VLC_CODEC_HEVC)
...
static void InvalidateAllPictures(decoder_t *p_dec)
{
    ...
    for (unsigned int i = 0; i < p_sys->video.i_inflight_pictures; ++i)
        AndroidOpaquePicture_Release(p_sys->video.pp_inflight_pictures[i], false);
}
...
static void RemoveInflightPictures(decoder_t *p_dec)
{
    ...
    TAB_CLEAN(p_sys->video.i_inflight_pictures, p_sys->video.pp_inflight_pictures);
}

static int Video_ProcessOutput(decoder_t *p_dec, mc_api_out *p_out, picture_t **pp_out_pic, block_t **pp_out_block)
{
    decoder_sys_t *p_sys = p_dec->p_sys;
    (void) pp_out_block;
    assert(pp_out_pic);
    ...
    else
    {
        ...
        if (!p_sys->api.b_direct_rendering && !GetVlcChromaFormat(p_sys->video.i_pixel_format, &p_dec->fmt_out.i_codec, &name))
        {
        ...
        int i_width  = p_out->conf.video.crop_right + 1 - p_out->conf.video.crop_left;
        int i_height = p_out->conf.video.crop_bottom + 1 - p_out->conf.video.crop_top;
...
static int Audio_ProcessOutput(decoder_t *p_dec, mc_api_out *p_out, picture_t **pp_out_pic, block_t **pp_out_block)
{
    decoder_sys_t *p_sys = p_dec->p_sys;
    (void) pp_out_pic;
    assert(pp_out_block);
    ...
    if (p_out->type == MC_OUT_TYPE_BUF)
    {
        ...
        if (!p_block)
            return -1;
        p_block->i_nb_samples = p_out->buf.i_size / p_dec->fmt_out.audio.i_bytes_per_frame;
        ...
        p_block->i_length = date_Increment(&p_sys->audio.i_end_date, p_block->i_nb_samples) - p_block->i_pts;
    ...
    else
    {
        ...
        if (p_out->conf.audio.channel_count <= 0 || p_out->conf.audio.channel_count > 8 || p_out->conf.audio.sample_rate <= 0)
        {
        ...
        p_sys->audio.b_extract = aout_CheckChannelExtraction(p_sys->audio.pi_extraction,
            &i_layout_dst, &i_channels_dst,
            NULL, pi_audio_order_src,
            p_sys->audio.i_channels);

        if (p_sys->audio.b_extract)
            msg_Warn(p_dec, "need channel extraction: %d -> %d", p_sys->audio.i_channels, i_channels_dst);
...
static void *OutThread(void *data)
{
    ...
    msg_Dbg(NULL, "KCY: OutThread started");
    ...
    for (;;)
    {
        ...
        if (i_index >= 0 || i_index == MC_API_INFO_OUTPUT_FORMAT_CHANGED || i_index == MC_API_INFO_OUTPUT_BUFFERS_CHANGED)
        {
            ...
            if (i_ret == 1)
            {
                ...
                if (p_sys->pf_process_output(p_dec, &out, &p_pic, &p_block) == -1 && !out.b_eos)
                {
...
static int QueueBlockLocked(decoder_t *p_dec, block_t *p_in_block, bool b_drain)
{
    decoder_sys_t *p_sys = p_dec->p_sys;
    block_t *p_block = NULL;
    ...
    while (b_drain || (p_block = GetNextBlock(p_sys, p_in_block)))
    {
        ...
        i_index = p_sys->api.dequeue_in(&p_sys->api, p_sys->api.b_direct_rendering ? INT64_C(2000000) : -1);
        vlc_mutex_lock(&p_sys->lock);
        ...
        if (i_index >= 0)
        {
            ...
            //msg_Dbg(p_dec, "KCY: queue_in(INDEX=%d, TS=%lld, BUFFER=%p, SIZE=%d, CONFIG=%d)", i_index, i_ts, p_buf, i_size, b_config);

            if (p_sys->api.queue_in(&p_sys->api, i_index, p_buf, i_size, i_ts, b_config) == 0)
            {
    ...
    if (b_drain)
    {
        ...
        while (!p_sys->b_aborted && !p_sys->b_drained && vlc_cond_timedwait(&p_sys->dec_cond, &p_sys->lock, deadline) == 0);
...
static int DecodeBlock(decoder_t *p_dec, block_t *p_in_block)
{
    ...
    if (p_in_block == NULL)
    {
        ...
        msg_Err(p_dec, "Decoder is draining (No input block)");

        /*if (p_sys->b_output_ready)
            QueueBlockLocked(p_dec, NULL, true);*/ // Commented by K.C.Y
        goto end;
    }

    if (p_in_block->i_flags & (BLOCK_FLAG_DISCONTINUITY | BLOCK_FLAG_CORRUPTED))
    {
        msg_Err(p_dec, "Block is discontinuity or corrupted (flags=%d)", p_in_block->i_flags);

        /*if (p_sys->b_output_ready)
            QueueBlockLocked(p_dec, NULL, true);
        DecodeFlushLocked(p_dec);*/ // Commented by K.C.Y
        if (p_sys->b_aborted)
            goto end;
    ...
    {
        msg_Warn(p_dec, "Draining from DecodeBlock(decoder_flags=%08X)", p_sys->i_decode_flags);
     ...
     * do it next time. */
    int ret = p_sys->b_aborted && p_sys->b_has_format ? VLCDEC_ECRITICAL : VLCDEC_SUCCESS;
    vlc_mutex_unlock(&p_sys->lock);
    return ret;
...
static int Video_OnNewBlock(decoder_t *p_dec, block_t **pp_block)
{
    ...
    timestamp_FifoPut(p_sys->video.timestamp_fifo, p_block->i_pts ? VLC_TS_INVALID : p_block->i_dts);
...
static int VideoHXXX_OnNewBlock(decoder_t *p_dec, block_t **pp_block)
{
    ...
    *pp_block = hh->pf_process_block(hh, *pp_block, p_config_changed);
    ...
    if (b_config_changed)
    {
        ...
        {
            if (p_sys->api.b_started)
                msg_Err(p_dec, "SPS/PPS changed during playback and video size are different. Restart it !");
            p_sys->i_decode_flags |= DECODE_FLAG_RESTART;
        }
        else
        {
            msg_Err(p_dec, "SPS/PPS changed during playback.");
            // p_sys->i_decode_flags |= DECODE_FLAG_DRAIN; Commented by K.C.Y
        }
    }
