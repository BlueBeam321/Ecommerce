...
#include "common/modules/packetizer/hevc_nal.h"
...
static int Start(mc_api *api, union mc_api_args *p_args)
{
    ...
    msg_Dbg(api->p_obj, "KCY: Creating codec (name=%s)", api->psz_name);
    ...
    {
        msg_Dbg(api->p_obj, "KCY: Creating video format (mime=%s, width=%d, height=%d, adaptive=%d)", 
            api->psz_mime, p_args->video.i_width, p_args->video.i_height, p_args->video.b_adaptive_playback);
...
static int GetOutput(mc_api *api, int i_index, mc_api_out *p_out)
{
    ...
    if (i_index >= 0)
    {
        ...
        p_out->buf.i_ts = (*env)->GetLongField(env, p_sys->buffer_info, jfields.pts_field);

        int flags = (*env)->GetIntField(env, p_sys->buffer_info, jfields.flags_field);
        ...
        else
        {
            ...
            if (jfields.get_output_buffers)
                buf = (*env)->GetObjectArrayElement(env, p_sys->output_buffers, i_index);
            else
            {
                buf = (*env)->CallObjectMethod(env, p_sys->codec, jfields.get_output_buffer, i_index);
                if (CHECK_EXCEPTION())
                {
            ...
            if (buf)
            {
                ...
                offset = (*env)->GetIntField(env, p_sys->buffer_info, jfields.offset_field);
            }
    ...
    else if (i_index == MC_API_INFO_OUTPUT_FORMAT_CHANGED)
    {
        ...
        msg_Dbg(api->p_obj, "output format changed: %.*s", format_len, format_ptr);
    ...
    else if (i_index == MC_API_INFO_OUTPUT_BUFFERS_CHANGED)
    {
        ...
        joutput_buffers = (*env)->CallObjectMethod(env, p_sys->codec, jfields.get_output_buffers);
        if (CHECK_EXCEPTION())
        {
            msg_Err(api->p_obj, "Exception in MediaCodec.getOutputBuffer");
            p_sys->output_buffers = NULL;
