...
#define MODULE_NAME     codec_libass
#define MODULE_STRING   "codec_libass"
...
static int Create( vlc_object_t *p_this )
{
    ...
    const char *psz_font = "/mnt/internal_sd/kpcholim_0.ttc";
    const char *psz_family = "KP CheonRiMa";
...
#else
    const char *psz_font = "C:\\Windows\\Fonts\\kpcholim_0.ttc"; /* We don't ship a default font with VLC */
    const char *psz_family = "KP CheonRiMa"; /* Use Arial if we can't find anything more suitable */
#endif
    ...
    mtime_t i_ts)
{
    ...
    for (int i = 0; i < i_region; i++)
    {
        ...
        /* */
        fmt_region = fmt;
        fmt_region.i_width = fmt_region.i_visible_width = region[i].x1 - region[i].x0;
        fmt_region.i_height = fmt_region.i_visible_height = region[i].y1 - region[i].y0;
...
static int BuildRegions( rectangle_t *p_region, int i_max_region, ASS_Image *p_img_list, int i_width, int i_height )
{
    ...
    int i_maxw = i_h_inc;
    int i_region;
    rectangle_t region[16/*i_max_region + 1*/];
