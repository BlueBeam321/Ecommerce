...
#define MODULE_NAME     codec_scte27
#define MODULE_STRING   "codec_scte27"
...
static subpicture_region_t *DecodeSimpleBitmap(decoder_t *dec, const uint8_t *data, int size)
{
    VLC_UNUSED(dec);
        ...
        if (!bs_read1(&bs))
        {
            if (!bs_read1(&bs))
            {
                if (!bs_read1(&bs))
                {
                    if (bs_read(&bs, 2) == 1)
                    {
                        int next = __MIN((position / bitmap_h + 1) * bitmap_h, bitmap_size);
    ...
    for (int y = 0; y < frame_v; y++)
        memset(&r->p_picture->p->p_pixels[y * r->p_picture->p->i_pitch], COLOR_FRAME, frame_h);
    ...
    if (outline_style == 1)
    {
        ...
        for (int dy = 0; dy <= 15; dy++)
        {
            for (int dx = 0; dx <= 15; dx++)
                circle[dy][dx] = (dx > 0 || dy > 0) && dx * dx + dy * dy <= outline_thickness * outline_thickness;
        }
    ...
    else if (outline_style == 2)
    {
        ...
        for (int by = 0; by < bitmap_v; by++)
        {
            ...
            {
                if (bitmap[by * bitmap_h + bx])
                    SetYUVPPixel(r->p_picture, bx + bitmap_oh + shadow_right, by + bitmap_ov + shadow_bottom, COLOR_SHADOW);
            }
        }
    }
    ...
    for (int by = 0; by < bitmap_v; by++)
    {
        ...
        {
            if (bitmap[by * bitmap_h + bx])
                SetYUVPPixel(r->p_picture, bx + bitmap_oh, by + bitmap_ov, COLOR_CHARACTER);
        }
    }
    free(bitmap);
    return r;
...
static subpicture_t *DecodeSubtitleMessage(decoder_t *dec, const uint8_t *data, int size, mtime_t date)
{
    if (size < 12)
        goto error;
...
static int Decode(decoder_t *dec, block_t *b)
{
    ...
    while (b->i_buffer > 3)
    {
        ...
        if (segmentation_overlay)
        {
            ...
            sys->segment_buffer = realloc(sys->segment_buffer, sys->segment_size + segment_size);
            memcpy(&sys->segment_buffer[sys->segment_size], &b->p_buffer[9], segment_size);
            sys->segment_size += segment_size;

            if (index == last)
            {
                sub = DecodeSubtitleMessage(dec, sys->segment_buffer, sys->segment_size, sys->segment_date);
                sys->segment_size = 0;
            }
        }
        else
            sub = DecodeSubtitleMessage(dec, &b->p_buffer[4], section_length - 1 - 4, b->i_pts > VLC_TS_INVALID ? b->i_pts : b->i_dts);
        if (sub != NULL)
