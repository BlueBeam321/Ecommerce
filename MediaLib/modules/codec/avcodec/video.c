 ...
 */
static int lavc_GetVideoFormat(decoder_t *dec, video_format_t * fmt,
...
static picture_t *DecodeBlock(decoder_t *p_dec, block_t **pp_block, bool *error)
{
    ...
    while (!p_block || p_block->i_buffer > 0 || eos_spotted)
    {
        ...
        if (p_context->pix_fmt == AV_PIX_FMT_PAL8 && !p_dec->fmt_out.video.p_palette)
        {
                            ...
                            picture_t *pic)
{
    ...
    for (int i = 0; i < pic->i_planes; i++)
    {
        ...
        {
            atomic_bool tmp_ret = atomic_exchange(&sys->b_dr_failure, true);
            if (!tmp_ret)
                msg_Warn(dec, "plane %d: pitch not aligned (%d%%%d): disabling direct rendering", i, pic->p[i].i_pitch, aligns[i]);
            goto error;
        }
