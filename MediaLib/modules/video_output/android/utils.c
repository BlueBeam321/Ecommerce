...
static inline void* NativeSurface_Load(const char *psz_lib, NativeSurface *p_ns)
{
    ...
    p_ns->pf_unlockAndPost = (AndroidSurface_unlockAndPost)(dlsym(p_lib, ANDROID_SYM_S_UNLOCK));
...
static void* NativeSurface_getHandle(JNIEnv *p_env, jobject jsurf)
{
    jclass clz;
    jfieldID fid;
...
static ANativeWindow* NativeSurface_fromSurface(JNIEnv *p_env, jobject jsurf)
{
    void *p_surface_handle;
    NativeSurface *p_ns;
...
static void NativeSurface_release(ANativeWindow* p_anw)
{
...
static int32_t NativeSurface_lock(ANativeWindow *p_anw, ANativeWindow_Buffer *p_anb, ARect *p_rect)
{
    (void) p_rect;
    NativeSurface *p_ns = (NativeSurface *)p_anw;
    struct {
...
static void NativeSurface_unlockAndPost(ANativeWindow *p_anw)
{
    NativeSurface *p_ns = (NativeSurface *)p_anw;
...
static void LoadNativeSurfaceAPI(AWindowHandler *p_awh)
{
...
int android_loadNativeWindowPrivApi(native_window_priv_api_t *native)
{
...
static JNIEnv* android_getEnvCommon(vlc_object_t *p_obj, JavaVM *jvm, const char *psz_name)
{
...
JNIEnv* android_getEnv(vlc_object_t *p_obj, const char *psz_name)
{
    return android_getEnvCommon(p_obj, NULL, psz_name);
}

static void AndroidNativeWindow_onMouseEvent(JNIEnv*, jobject, jlong, jint, jint, jint, jint);
static void AndroidNativeWindow_onWindowSize(JNIEnv*, jobject, jlong, jint, jint );
...
static int InitJNIFields(JNIEnv *env, vlc_object_t *p_obj, jobject *jobj)
{
...
#define GET_METHOD(id, str, args, critical) do { \
    ...
    GET_METHOD(AndroidNativeWindow.getVideoSurface, "getVideoSurface", "()Landroid/view/Surface;", true);
    GET_METHOD(AndroidNativeWindow.getSubtitlesSurface, "getSubtitlesSurface", "()Landroid/view/Surface;", true);
    GET_METHOD(AndroidNativeWindow.registerNative, "registerNative", "(J)I", true);
    GET_METHOD(AndroidNativeWindow.unregisterNative, "unregisterNative", "()V", true);
    GET_METHOD(AndroidNativeWindow.setBuffersGeometry, "setBuffersGeometry", "(Landroid/view/Surface;III)Z", true);
    GET_METHOD(AndroidNativeWindow.setVideoLayout, "setVideoLayout", "(IIIIII)V", true);

    GET_METHOD(SurfaceTexture.attachToGLContext, "SurfaceTexture_attachToGLContext", "(I)Z", true);
    GET_METHOD(SurfaceTexture.detachFromGLContext, "SurfaceTexture_detachFromGLContext", "()V", true);
    GET_METHOD(SurfaceTexture.waitAndUpdateTexImage, "SurfaceTexture_waitAndUpdateTexImage", "([F)Z", true);
    GET_METHOD(SurfaceTexture.getSurface, "SurfaceTexture_getSurface", "()Landroid/view/Surface;", true);
...
static JNIEnv* AWindowHandler_getEnv(AWindowHandler *p_awh)
{
...
#define AWINDOW_REGISTER_FLAGS_HAS_VIDEO_LAYOUT_LISTENER 0x2
    ...
    const jint flags = JNI_ANWCALL(CallIntMethod, registerNative, (jlong)(intptr_t)p_awh);
    ...
    p_awh->b_has_video_layout_listener = flags & AWINDOW_REGISTER_FLAGS_HAS_VIDEO_LAYOUT_LISTENER;

    return p_awh;
}

static void AWindowHandler_releaseANativeWindowEnv(AWindowHandler *p_awh, JNIEnv *p_env, enum AWindow_ID id)
{
...
static int WindowHandler_NewSurfaceEnv(AWindowHandler *p_awh, JNIEnv *p_env, enum AWindow_ID id)
{
    jobject jsurface;
...
ANativeWindow* AWindowHandler_getANativeWindow(AWindowHandler *p_awh, enum AWindow_ID id)
{
    assert(id < AWindow_Max);
    ...
    p_awh->views[id].p_anw = p_awh->pf_winFromSurface(p_env, p_awh->views[id].jsurface);
    return p_awh->views[id].p_anw;
}
...
void AWindowHandler_releaseANativeWindow(AWindowHandler *p_awh, enum AWindow_ID id)
{
    JNIEnv *p_env = AWindowHandler_getEnv(p_awh);
    if (p_env)
...
static void AndroidNativeWindow_onMouseEvent(JNIEnv* env, jobject clazz, jlong handle, jint action, jint button, jint x, jint y)
{
    (void) env; (void) clazz;
    AWindowHandler *p_awh = jlong_AWindowHandler(handle);

    p_awh->event.cb.on_new_mouse_coords(p_awh->wnd, &(struct awh_mouse_coords) { action, button, x, y });
}

static void AndroidNativeWindow_onWindowSize(JNIEnv* env, jobject clazz, jlong handle, jint width, jint height)
{
    (void) env; (void) clazz;
    AWindowHandler *p_awh = jlong_AWindowHandler(handle);
...
int AWindowHandler_setBuffersGeometry(AWindowHandler *p_awh, enum AWindow_ID id, int i_width, int i_height, int i_format)
{
    jobject jsurf;
    JNIEnv *p_env = AWindowHandler_getEnv(p_awh);
    if (!p_env)
    ...
    return JNI_ANWCALL(CallBooleanMethod, setBuffersGeometry,
        jsurf, i_width, i_height, i_format) ? VLC_SUCCESS : VLC_EGENERIC;;
}
...
int SurfaceTexture_attachToGLContext(AWindowHandler *p_awh, int tex_name)
{
    JNIEnv *p_env = android_getEnvCommon(NULL, p_awh->p_jvm, "SurfaceTexture");
    if (!p_env)
        return VLC_EGENERIC;

    return JNI_STEXCALL(CallBooleanMethod, attachToGLContext, tex_name) ? VLC_SUCCESS : VLC_EGENERIC;
}

void SurfaceTexture_detachFromGLContext(AWindowHandler *p_awh)
{
    JNIEnv *p_env = android_getEnvCommon(NULL, p_awh->p_jvm, "SurfaceTexture");
    if (!p_env)
        return;
...
int SurfaceTexture_waitAndUpdateTexImage(AWindowHandler *p_awh, const float **pp_transform_mtx)
{
    JNIEnv *p_env = android_getEnvCommon(NULL, p_awh->p_jvm, "SurfaceTexture");
    if (!p_env)
        return VLC_EGENERIC;
    ...
    if (p_awh->stex.jtransform_mtx != NULL)
        (*p_env)->ReleaseFloatArrayElements(p_env, p_awh->stex.jtransform_mtx_array, p_awh->stex.jtransform_mtx, JNI_ABORT);

    bool ret = JNI_STEXCALL(CallBooleanMethod, waitAndUpdateTexImage, p_awh->stex.jtransform_mtx_array);
    if (ret)
    {
        p_awh->stex.jtransform_mtx = (*p_env)->GetFloatArrayElements(p_env, p_awh->stex.jtransform_mtx_array, NULL);
