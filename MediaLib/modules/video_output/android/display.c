...
#define MODULE_NAME     vout_android
#define MODULE_STRING   "vout_android"
...
static int UpdateVideoSize(vout_display_sys_t *sys, video_format_t *p_fmt, bool b_cropped)
{
    ...
    AWindowHandler_setVideoLayout(sys->p_awh,
        i_width, i_height,
        rot_fmt.i_visible_width, rot_fmt.i_visible_height,
        i_sar_num, i_sar_den);
    return 0;
}
...
static void FixSubtitleFormat(vout_display_sys_t *sys)
{
    ...
    p_subfmt->i_height = p_subfmt->i_visible_height = i_height;
...
static void SetupPictureYV12(picture_t *p_picture, uint32_t i_in_stride)
{
    ...
    if (vlc_fourcc_AreUVPlanesSwapped(p_picture->format.i_chroma, VLC_CODEC_YV12))
    {
...
static void AndroidWindow_DisconnectSurface(vout_display_sys_t *sys, android_window *p_window)
{
...
static int AndroidWindow_ConnectSurface(vout_display_sys_t *sys, android_window *p_window)
{
    if (!p_window->p_surface)
    {
        p_window->p_surface = AWindowHandler_getANativeWindow(sys->p_awh, p_window->id);
        if (!p_window->p_surface)
            return -1;
        if (p_window->b_opaque)
            p_window->p_jsurface = AWindowHandler_getSurface(sys->p_awh, p_window->id);
    }

    return 0;
}

static android_window *AndroidWindow_New(vout_display_t *vd, video_format_t *p_fmt, enum AWindow_ID id, bool b_use_priv)
{
...
static void AndroidWindow_Destroy(vout_display_t *vd, android_window *p_window)
{
    AndroidWindow_DisconnectSurface(vd->sys, p_window);
    free(p_window);
}

static int AndroidWindow_UpdateCrop(vout_display_sys_t *sys, android_window *p_window)
{
    if (!p_window->p_surface_priv)
        return -1;
...
static int AndroidWindow_SetupANWP(vout_display_sys_t *sys, android_window *p_window, bool b_java_configured)
{
    ...
    if (!b_java_configured
        && sys->anwp.setBuffersGeometry(p_window->p_surface_priv, p_window->fmt.i_width, p_window->fmt.i_height, p_window->i_android_hal) != 0)
        goto error;

    sys->anwp.getMinUndequeued(p_window->p_surface_priv, &p_window->i_min_undequeued);

    sys->anwp.getMaxBufferCount(p_window->p_surface_priv, &i_max_buffer_count);

    if ((p_window->i_min_undequeued + p_window->i_pic_count) > i_max_buffer_count)
        p_window->i_pic_count = i_max_buffer_count - p_window->i_min_undequeued;

    if (sys->anwp.setBufferCount(p_window->p_surface_priv, p_window->i_pic_count + p_window->i_min_undequeued) != 0)
        goto error;

    if (sys->anwp.setOrientation(p_window->p_surface_priv, p_window->i_angle) != 0)
        goto error;
...
static int AndroidWindow_ConfigureJavaSurface(vout_display_sys_t *sys, android_window *p_window, bool *p_java_configured)
{
    if (AWindowHandler_setBuffersGeometry(sys->p_awh, p_window->id, p_window->fmt.i_width, p_window->fmt.i_height, p_window->i_android_hal) == VLC_SUCCESS)
    {
...
static int AndroidWindow_SetupANW(vout_display_sys_t *sys, android_window *p_window, bool b_java_configured)
{
    p_window->i_pic_count = 1;
    p_window->i_min_undequeued = 0;

    if (!b_java_configured && sys->anw->setBuffersGeometry)
        return sys->anw->setBuffersGeometry(p_window->p_surface, p_window->fmt.i_width, p_window->fmt.i_height, p_window->i_android_hal);
    
    return 0;
}

static int AndroidWindow_Setup(vout_display_sys_t *sys, android_window *p_window, unsigned int i_pic_count)
{
    ...
    if (!p_window->b_opaque)
    {
        ...
        if (AndroidWindow_ConfigureJavaSurface(sys, p_window, &b_java_configured) != 0)
            return -1;

        if (!p_window->b_use_priv || AndroidWindow_SetupANWP(sys, p_window, b_java_configured) != 0)
        {
...
static void AndroidWindow_UnlockPicture(vout_display_sys_t *sys, android_window *p_window, picture_t *p_pic, bool b_render)
{
...
static int AndroidWindow_LockPicture(vout_display_sys_t *sys, android_window *p_window, picture_t *p_pic)
{
    picture_sys_t *p_picsys = p_pic->p_sys;
    ...
    if (p_window->b_use_priv)
    {
        ...
        err = sys->anwp.lockData(p_window->p_surface_priv, &p_handle, &p_picsys->sw.buf);
        if (err != 0)
            return -1;
    ...
    {
        if (sys->anw->winLock(p_window->p_surface, &p_picsys->sw.buf, NULL) != 0)
            return -1;
    }
...
static int OpenCommon(vout_display_t *vd)
{
    ...
    vout_window_t *embed = vout_display_NewWindow(vd, VOUT_WINDOW_TYPE_ANDROID_NATIVE);
    if (embed == NULL)
        return VLC_EGENERIC;
    ...
    msg_Dbg(vd, "using %s", sys->p_window->b_opaque ? "opaque" : (sys->p_window->b_use_priv ? "ANWP" : "ANW"));
    ...
    {
        msg_Warn(vd, "cannot blend subtitles with an opaque surface, trying next vout");
        //goto error;
    }
...
static int Open(vlc_object_t *p_this)
{
    ...
    msg_Dbg(NULL, "Android VOUT DISPLAY Open");
    if (vd->fmt.i_chroma == VLC_CODEC_ANDROID_OPAQUE)
        return VLC_EGENERIC;
...
static void ClearSurface(vout_display_t *vd)
{
    ...
    else
    {
        ...
        if (sys->anw->setBuffersGeometry(p_window->p_surface, 1, 1, WINDOW_FORMAT_RGB_565) == 0
...
static picture_pool_t *PoolAlloc(vout_display_t *vd, unsigned requested_count)
{
    ...
    {
        picture_t *p_pic = PictureAlloc(sys, &sys->p_window->fmt, sys->p_window->b_opaque);
        if (!p_pic)
            goto error;
...
static void SubtitleRegionToBounds(subpicture_t *subpicture, ARect *p_out_bounds)
{
    if (subpicture)
    {
...
static void SubtitleGetDirtyBounds(vout_display_t *vd, subpicture_t *subpicture, ARect *p_out_bounds)
{
    vout_display_sys_t *sys = vd->sys;
    int i = 0;
    bool b_found = false;
    ...
    {
        buffer_bounds *p_bb = realloc(sys->p_sub_buffer_bounds, (i + 2) * sizeof(buffer_bounds));
        if (p_bb)
        {
...
static void SubpicturePrepare(vout_display_t *vd, subpicture_t *subpicture)
{
    ...
    const int x_pixels_offset = memset_bounds.left * sys->p_sub_pic->p[0].i_pixel_pitch;
    const int i_line_size = (memset_bounds.right - memset_bounds.left) * sys->p_sub_pic->p->i_pixel_pitch;
...
static void Prepare(vout_display_t *vd, picture_t *picture, subpicture_t *subpicture)
{
    ...
    if (subpicture && sys->p_sub_window)
    {
        ...
        if (!sys->p_sub_pic && AndroidWindow_Setup(sys, sys->p_sub_window, 1) == 0)
    ...
    if (sys->p_window->b_opaque && AndroidOpaquePicture_CanReleaseAtTime(picture->p_sys))
    {
        mtime_t now = mdate();
        if (picture->date > now)
        {
...
static void Display(vout_display_t *vd, picture_t *picture, subpicture_t *subpicture)
{
    ...
    if (sys->p_sub_pic && sys->p_sub_window)
        AndroidWindow_UnlockPicture(sys, sys->p_sub_window, sys->p_sub_pic, true);
...
static void CopySourceAspect(video_format_t *p_dest, const video_format_t *p_src)
{
...
static int Control(vout_display_t *vd, int query, va_list args)
{
    ...
    case VOUT_DISPLAY_CHANGE_DISPLAY_SIZE:
    {
        ...
        msg_Dbg(vd, "change display size: %dx%d", sys->i_display_width, sys->i_display_height);
        FixSubtitleFormat(sys);
        return VLC_SUCCESS;
    }
