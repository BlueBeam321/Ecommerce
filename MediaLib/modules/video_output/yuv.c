...
#define MODULE_NAME     vout_yuv
#define MODULE_STRING   "vout_yuv"
...
vlc_module_begin()
    ...
    add_string(CFG_PREFIX "file", "stream.yuv", YUV_FILE_TEXT, YUV_FILE_LONGTEXT, false)
    add_string(CFG_PREFIX "chroma", NULL, CHROMA_TEXT, CHROMA_LONGTEXT, true)
    add_bool  (CFG_PREFIX "yuv4mpeg2", false, YUV4MPEG2_TEXT, YUV4MPEG2_LONGTEXT, true)
    add_shortcut("yuv")
    ...
    for (int i = 0; i < picture->i_planes; i++) {
        ...
        for( int y = 0; y < plane->i_visible_lines; y++) {
            ...
            if (written != (size_t)plane->i_visible_pitch)
                msg_Warn(vd, "only %d of %d bytes written",
