 ...
 *         Not RFC 2326 compliant yet and only handle REAL RTSP.
 *****************************************************************************/
...
static int rtsp_put( rtsp_client_t *rtsp, const char *psz_string )
{
    ...
    uint8_t *psz_buffer = malloc( i_buffer + 2 );
    int i_ret;
...
int rtsp_read_data( rtsp_client_t *rtsp, uint8_t *buffer, unsigned int size )
{
    ...
    if( size >= 4 )
    {
            ...
            buffer[3]=='_' )
        {
            ...
            rest = malloc(sizeof("Cseq: ") + 3 * sizeof(int));
                  ...
                  const char *psz_user_agent )
{
    ...
    if( colon != slash )
    {
        char buffer[256/*pathbegin-hostend*/];
