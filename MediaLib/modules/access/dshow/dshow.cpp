...
#include "access.h"
#include "filter.h"
#include "mta_holder.h"

#define MODULE_NAME     access_demux_dshow
#define MODULE_STRING   "access_demux_dshow"
...
static ComPtr<IBaseFilter> FindCaptureDevice(vlc_object_t *, std::string *, std::list<std::string> *, bool);
static size_t EnumDeviceCaps(vlc_object_t *, IBaseFilter *, int, int, int, int, int, int, AM_MEDIA_TYPE *mt, size_t, bool);
...
static const char *const ppsz_tuner_input_text[] = { N_("Default"), N_("Cable"), N_("Antenna") };
...
static void CreateDirectShowGraph(access_sys_t *p_sys)
{
    ...
    if (SUCCEEDED(CoCreateInstance(CLSID_FilterGraph, 0, 
        CLSCTX_INPROC, (REFIID)IID_IFilterGraph, 
        (void**)&p_sys->p_graph)))
    {
        ...
        {
            p_sys->p_capture_graph_builder2->SetFiltergraph((IGraphBuilder *)p_sys->p_graph);
        }

        p_sys->p_graph->QueryInterface(IID_IMediaControl, (void**)&p_sys->p_control);
    }
}
...
static void DeleteDirectShowGraph(vlc_object_t *p_this, access_sys_t *p_sys)
{
    ...
    for (i = 0; i < p_sys->pp_streams.size(); i++)
    {
        dshow_stream_t* p_stream = p_sys->pp_streams.at(i);
        ...
        p_sys->p_graph->RemoveFilter(p_stream->p_capture_filter.Get());
        p_stream->p_capture_filter.Reset();

        msg_Dbg(p_this, "DeleteDirectShowGraph: Removing device filter");
        p_sys->p_graph->RemoveFilter(p_stream->p_device_filter.Get());
        p_stream->p_device_filter.Reset();
    }
 ...
 *****************************************************************************/
static int CommonOpen(vlc_object_t *p_this, access_sys_t *p_sys, bool b_access_demux)
{
    char *psz_val;
    ...
    var_Create(p_this, CFG_PREFIX "tuner-channel", VLC_VAR_INTEGER | VLC_VAR_DOINHERIT);
    var_Create(p_this, CFG_PREFIX "tuner-frequency", VLC_VAR_INTEGER | VLC_VAR_DOINHERIT);
    var_Create(p_this, CFG_PREFIX "tuner-standard", VLC_VAR_INTEGER | VLC_VAR_DOINHERIT);
    var_Create(p_this, CFG_PREFIX "tuner-country", VLC_VAR_INTEGER | VLC_VAR_DOINHERIT);
    var_Create(p_this, CFG_PREFIX "tuner-input", VLC_VAR_INTEGER | VLC_VAR_DOINHERIT);

    var_Create(p_this, CFG_PREFIX "amtuner-mode", VLC_VAR_INTEGER | VLC_VAR_DOINHERIT);
    ...
    {
        msg_Warn(p_this, "Capture failed, No video or audio device selected.");
        return VLC_EGENERIC ;
    }
    ...
    if (b_use_video && !b_err_video)
    {
         ...
         * a demuxer module */
        if (p_sys->pp_streams.size() > 0)
        {
            dshow_stream_t* p_stream = p_sys->pp_streams.at(p_sys->pp_streams.size() - 1);
    ...
    for (int i = p_sys->i_crossbar_route_depth - 1; i >= 0; --i)
    {
        ...
        {
            msg_Dbg(p_this, "crossbar at depth %d, routed video output %ld to video input %ld",
                i,
                VideoOutputIndex,
                VideoInputIndex);
            ...
            {
                if (SUCCEEDED(pXbar->Route(AudioOutputIndex, AudioInputIndex)))
                {
            ...
            msg_Err(p_this, "crossbar at depth %d could not route video output %ld to input %ld", i, VideoOutputIndex, VideoInputIndex);
    }
    ...
    */
    if (var_GetBool(p_this, CFG_PREFIX "config"))
    {
        for (int i = p_sys->i_crossbar_route_depth - 1; i >= 0; --i)
        {
            ...
            if (SUCCEEDED(pXbar->QueryInterface(IID_IBaseFilter, (void**)p_XF.ReleaseAndGetAddressOf())))
    ...
    for (i = 0; i < p_sys->pp_streams.size(); i++)
    {
        ...
        if (p_stream->mt.majortype == MEDIATYPE_Video)
        {
            ...
            if (p_stream->header.video.AvgTimePerFrame)
            {
                ...
                fmt.video.i_frame_rate_base = p_stream->header.video.AvgTimePerFrame;
            }
        }
    ...
    CaptureFilter *p_capture_filter)
{
    ...
    if (p_sys->p_capture_graph_builder2)
    {
        if (FAILED(p_sys->p_capture_graph_builder2->RenderStream(&PIN_CATEGORY_CAPTURE, &mediaType.majortype, p_filter, 0, (IBaseFilter *)p_capture_filter)))
            return false;
            ...
            while (!Found && (S_OK == pins->Next(1, pP.ReleaseAndGetAddressOf(), &n)))
            {
                if (S_OK == pP->QueryPinInfo(&pinInfo))
                {
                    ...
                    if (pinInfo.dir == PINDIR_INPUT &&
                        pP.Get()->QueryInterface(IID_IKsPropertySet, (void**)pKs.GetAddressOf()) == S_OK)
                    {
    ...
    else
    {
        ...
        if (S_OK != p_filter->EnumPins(p_enumpins.GetAddressOf()))
            return false;
        ...
        while (S_OK == p_enumpins->Next(1, p_pin.ReleaseAndGetAddressOf(), NULL))
        {
            ...
            if (pin_dir == PINDIR_OUTPUT && p_sys->p_graph->ConnectDirect(p_pin.Get(), p_input_pin.Get(), 0) == S_OK)
                return true;
        }
        return false;
    }
...
static int OpenDevice(vlc_object_t *p_this, access_sys_t *p_sys, std::string devicename, bool b_audio)
{
    ComPtr<IBaseFilter> p_device_filter;
    unsigned int i;
    ...
    {
        dshow_stream_t* p_stream = p_sys->pp_streams.at(i);
        if (devicename.size() && p_stream->devicename == devicename)
        {
            /* Already opened */
    ...
    if (!p_device_filter)
    {
        ...
        {
            msg_Err(p_this, "can't use device: %s, unsupported device type", devicename.c_str());
            return VLC_EGENERIC;
        }
    ...
    size_t media_count = EnumDeviceCaps(p_this, p_device_filter.Get(), b_audio ? 0 : p_sys->i_chroma,
    ...
    ComPtr<CaptureFilter> p_capture_filter(new CaptureFilter(p_this, p_sys, mt, media_count));
    ...
    if (ConnectFilters(p_this, p_sys, p_device_filter.Get(), p_capture_filter.Get()))
    {
        ...
        dshow_stream.p_es = 0;
        dshow_stream.mt = p_capture_filter->CustomGetPin()->CustomGetMediaType();

        /* Show Device properties. Done here so the VLC stream is setup with
         * the proper parameters. */
        if (var_GetBool(p_this, CFG_PREFIX "config"))
        {
            ShowDeviceProperties(p_this,
                p_sys->p_capture_graph_builder2,
        ...
        if (dshow_stream.i_fourcc)
        {
            ...
            {
                dshow_stream.header.video = *(VIDEOINFOHEADER *)dshow_stream.mt.pbFormat;
            ...
            {
                dshow_stream.header.audio = *(WAVEFORMATEX *)dshow_stream.mt.pbFormat;
...
*/
static ComPtr<IBaseFilter> FindCaptureDevice(vlc_object_t *p_this, std::string *p_devicename, std::list<std::string> *p_listdevices, bool b_audio)
{
    ...
    hr = CoCreateInstance(CLSID_SystemDeviceEnum, NULL, CLSCTX_INPROC, IID_ICreateDevEnum, (void**)p_dev_enum.GetAddressOf());
    if (FAILED(hr))
    {
    ...
    if (!b_audio)
        hr = p_dev_enum->CreateClassEnumerator(CLSID_VideoInputDeviceCategory, p_class_enum.GetAddressOf(), 0);
    else
        hr = p_dev_enum->CreateClassEnumerator(CLSID_AudioInputDeviceCategory, p_class_enum.GetAddressOf(), 0);
    if (FAILED(hr))
    {
    ...
    while (p_class_enum->Next(1, p_moniker.ReleaseAndGetAddressOf(), &i_fetched) == S_OK)
    {
        ...
        hr = p_moniker->BindToStorage(0, 0, IID_IPropertyBag, (void**)p_bag.GetAddressOf());
        if (SUCCEEDED(hr))
        {
            VARIANT var;
            var.vt = VT_BSTR;
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (p_devicename && *p_devicename == devname)
                {
                    ...
                    hr = p_moniker->BindToObject(NULL, 0, IID_IBaseFilter, (void**)p_base_filter.GetAddressOf());
                    if (FAILED(hr))
                    {
    ...
    size_t mt_max, bool b_audio)
{
    ...
    while (!mt_count && p_enumpins->Next(1, p_output_pin.ReleaseAndGetAddressOf(), NULL) == S_OK)
    {
        ...
        */
        ...
        if (SUCCEEDED(p_output_pin.Get()->QueryInterface(IID_IAMStreamConfig, (void**)pSC.GetAddressOf())))
        {
            int piCount, piSize;
                    ...
                    for (int i = 0; i < piCount; ++i)
                    {
                        if (SUCCEEDED(pSC->GetStreamCaps(i, &p_mt, pSCC)))
                        {
                                ...
                                && FORMAT_VideoInfo == p_mt->formattype)
                            {
                                ...
                                if (i_width)
                                {
                                    if ((!pVSCC->OutputGranularityX && i_width != pVSCC->MinOutputSize.cx && i_width != pVSCC->MaxOutputSize.cx)
                                        || (pVSCC->OutputGranularityX && ((i_width % pVSCC->OutputGranularityX) || pVSCC->MinOutputSize.cx > i_width || i_width > pVSCC->MaxOutputSize.cx)))
                                    {
                                ...
                                if (i_height)
                                {
                                    if ((!pVSCC->OutputGranularityY && i_height != pVSCC->MinOutputSize.cy && i_height != pVSCC->MaxOutputSize.cy)
                                        || (pVSCC->OutputGranularityY && ((i_height % pVSCC->OutputGranularityY) || pVSCC->MinOutputSize.cy > i_height || i_height > pVSCC->MaxOutputSize.cy)))
                                    {
                                ...
                                p_mt->lSampleSize = pVih->bmiHeader.biSizeImage = ((pVih->bmiHeader.biWidth + 3) & ~3) * pVih->bmiHeader.biHeight * (pVih->bmiHeader.biBitCount >> 3);
                                ...
                                && p_mt->formattype == FORMAT_WaveFormatEx)
                            {
                                ...
                                if (i_current_fourcc && (WAVE_FORMAT_PCM == pWfx->wFormatTag))
                                {
                                    ...
                                    if ((!pASCC->ChannelsGranularity && (unsigned int)val != pASCC->MinimumChannels && (unsigned int)val != pASCC->MaximumChannels)
                                        || (pASCC->ChannelsGranularity && ((val % pASCC->ChannelsGranularity) || (unsigned int)val < pASCC->MinimumChannels || (unsigned int)val > pASCC->MaximumChannels)))
                                    {
                                    ...
                                    if ((!pASCC->SampleFrequencyGranularity && (unsigned int)val != pASCC->MinimumSampleFrequency && (unsigned int)val != pASCC->MaximumSampleFrequency)
                                        || (pASCC->SampleFrequencyGranularity && ((val % pASCC->SampleFrequencyGranularity) || (unsigned int)val < pASCC->MinimumSampleFrequency || (unsigned int)val > pASCC->MaximumSampleFrequency)))
                                    {
                                    ...
                                    if ((!pASCC->BitsPerSampleGranularity && (unsigned int)val != pASCC->MinimumBitsPerSample && (unsigned int)val != pASCC->MaximumBitsPerSample)
                                        || (pASCC->BitsPerSampleGranularity && ((val % pASCC->BitsPerSampleGranularity) || (unsigned int)val < pASCC->MinimumBitsPerSample || (unsigned int)val > pASCC->MaximumBitsPerSample)))
                                    {
        ...
        */
        ...
        while (p_enummt->Next(1, &p_mt, NULL) == S_OK)
        {
                ...
                && p_mt->formattype == FORMAT_VideoInfo)
            {
                ...
                && p_mt->formattype == FORMAT_WaveFormatEx)
            {
                int i_current_channels = ((WAVEFORMATEX *)p_mt->pbFormat)->nChannels;
                int i_current_samplespersec = ((WAVEFORMATEX *)p_mt->pbFormat)->nSamplesPerSec;
                int i_current_bitspersample = ((WAVEFORMATEX *)p_mt->pbFormat)->wBitsPerSample;
                ...
                if ((!i_channels || i_channels == i_current_channels) &&
                    (!i_samplespersec || i_samplespersec == i_current_samplespersec) &&
                    (!i_bitspersample || i_bitspersample == i_current_bitspersample) &&
                    mt_count < mt_max)
                {
                    /* Pick  match */
                    mt[mt_count++] = *p_mt;
                    ...
                    if (SUCCEEDED(p_output_pin.Get()->QueryInterface(IID_IAMBufferNegotiation, (void **)p_ambuf.GetAddressOf())))
                    {
            ...
            else if (i_current_fourcc && p_mt->majortype == MEDIATYPE_Stream)
            {
                ...
                if ((!i_fourcc || i_fourcc == i_current_fourcc) && mt_count < mt_max)
                {
                    /* Pick match */
                    mt[mt_count++] = *p_mt;
...
static int Demux(demux_t *p_demux)
{
    ...
    while (!i_found_samples)
    {
        ...
        for (i = 0; i < p_sys->pp_streams.size(); i++)
        {
            ...
            if (p_stream->p_capture_filter &&
                p_stream->p_capture_filter->CustomGetPin()->CustomGetSamples(p_stream->samples_queue) == S_OK)
            {
                i_found_samples = 1;
            }
        }
    ...
    for (i = 0; i < p_sys->pp_streams.size(); i++)
    {
        ...
        while (i_samples > 0)
        {
            ...
            {
                i_pts += (i_pts >= 0) ? +5 : -4;
                i_pts /= 10; /* 100-ns to us conversion */
                i_pts += VLC_TS_0;
            }
#if 0
...
static int AccessControl(stream_t *p_access, int i_query, va_list args)
{
    ...
    case STREAM_GET_PTS_DELAY:
        ...
        *pi_64 = INT64_C(1000) * var_InheritInteger(p_access, "live-caching");
        break;
...
static int AppendAudioEnabledVDevs(vlc_object_t *p_this, std::list<std::string> &audio_list, std::list<std::string> &video_list)
{
    ComPtr<IFilterGraph> p_graph;
    ComPtr<IGraphBuilder> p_gbuilder;
    ComPtr<ICaptureGraphBuilder2> p_cgbuilder;

    if (FAILED(CoCreateInstance(CLSID_FilterGraph, NULL, CLSCTX_INPROC_SERVER, IID_IFilterGraph, (void **)p_graph.GetAddressOf())))
        return VLC_EGENERIC;

    if (FAILED(p_graph.Get()->QueryInterface(IID_IGraphBuilder, (void **)p_gbuilder.GetAddressOf())))
        return VLC_EGENERIC;

    if (FAILED(CoCreateInstance(CLSID_CaptureGraphBuilder2, NULL, CLSCTX_INPROC_SERVER, IID_ICaptureGraphBuilder2, (void **)p_cgbuilder.GetAddressOf())))
        return VLC_EGENERIC;

    if (FAILED(p_cgbuilder->SetFiltergraph(p_gbuilder.Get())))
        return VLC_EGENERIC;

    for (std::list<std::string>::iterator iter = video_list.begin(); iter != video_list.end(); ++iter)
    {
        ...
        if (SUCCEEDED(p_cgbuilder->FindPin(p_device.Get(), PINDIR_OUTPUT, NULL, &MEDIATYPE_Audio, true, 0, p_pin.GetAddressOf())))
 ...
 *****************************************************************************/
static int FindDevices(vlc_object_t *p_this, const char *psz_name, char ***vp, char ***tp)
{
    ...
    char **values = (char **)malloc(count * sizeof(*values));
    char **texts = (char **)malloc(count * sizeof(*texts));
    ...
    for (std::list<std::string>::iterator iter = list_devices.begin(); iter != list_devices.end(); ++iter)
    {
        assert(i < count);
...
static void ShowPropertyPage(IUnknown *obj)
{
    ...
    HRESULT hr = obj->QueryInterface(IID_ISpecifyPropertyPages, (void **)p_spec.GetAddressOf());
    if (FAILED(hr))
        return;
    ...
    if (SUCCEEDED(p_spec->GetPages(&cauuid)))
    {
        if (cauuid.cElems > 0)
        {
            ...
            OleCreatePropertyFrame(hwnd_desktop, 30, 30, NULL, 1, &obj, cauuid.cElems, cauuid.pElems, 0, 0, NULL);
...
static void ConfigTuner(vlc_object_t *p_this, ICaptureGraphBuilder2 *p_graph, IBaseFilter *p_device_filter)
{
    ...
    if (i_frequency <= 0)
        p_TV->put_Channel(i_channel, AMTUNER_SUBCHAN_NO_TUNE, AMTUNER_SUBCHAN_NO_TUNE);
    ...
    if (i_frequency > 0 || i_standard > 0)
    {
        ...
        hr = p_TV.Get()->QueryInterface(IID_IKsPropertySet, (void **)pKs.GetAddressOf());
        if (FAILED(hr))
        {
