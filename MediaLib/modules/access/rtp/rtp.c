...
#define MODULE_NAME     access_demux_rtp
#define MODULE_STRING   "access_demux_rtp"
...
#if CRYPT_ENABLE
static const char *demux_name = "TT378HD3JV1J1RQ0HXU0E8JXVV";
#else
static const char *demux_name = "ts";
#endif
...
static int Open(vlc_object_t *obj)
{
    ...
    if (!strcmp(demux->psz_access, "dccp"))
        tp = IPPROTO_DCCP;
    else if (!strcmp(demux->psz_access, "rtptcp"))
        tp = IPPROTO_TCP;
    else if (!strcmp(demux->psz_access, "rtp"))
        tp = IPPROTO_UDP;
    else if (!strcmp(demux->psz_access, "udplite"))
        tp = IPPROTO_UDPLITE;
    else
        return VLC_EGENERIC;
...
#ifdef MANUAL_IGMP
        host = inet_addr(dhost); // Added by K.C.Y
#endif
...
#define SOCK_DCCP 6
...
#ifdef MANUAL_IGMP
    p_sys->igmp_fd = net_IGMPOpen(); // Added by K.C.Y
    p_sys->host = ntohl(host); // Added by K.C.Y
    p_sys->port = dport; // Added by K.C.Y
#endif
    p_sys->max_src = var_CreateGetInteger(obj, "rtp-max-src");
    p_sys->timeout = var_CreateGetInteger(obj, "rtp-timeout") * CLOCK_FREQ;
    ...
    {
        vlc_gcrypt_init();
        p_sys->srtp = srtp_create(SRTP_ENCR_AES_CM, SRTP_AUTH_HMAC_SHA1, 10, SRTP_PRF_AES_CM, SRTP_RCC_MODE1);
        if (p_sys->srtp == NULL)
        {
            free(key);
            goto error;
        ...
        if (val)
        {
            msg_Err(obj, "bad SRTP key/salt combination (%s)", vlc_strerror_c(val));
            goto error;
        }
    }
#endif

    if (vlc_clone(&p_sys->thread, (tp != IPPROTO_TCP) ? rtp_dgram_thread : rtp_stream_thread, demux, "access_rtp_read", VLC_THREAD_PRIORITY_INPUT))
        goto error;
...
#ifdef MANUAL_IGMP
    if (p_sys->igmp_fd != -1) // Added by K.C.Y
        net_IGMPClose(p_sys->igmp_fd);
#endif
    net_Close(p_sys->fd);
    free(p_sys);
}
...
static void *ts_init (demux_t *demux)
{
    ...
    return stream_init (demux, name ? name : demux_name);
}
...
void rtp_autodetect(demux_t *demux, rtp_session_t *session, const block_t *block)
{
    ...
    default:
         ...
         */
        if (!strcmp(demux->psz_demux, "h264") || !strcmp(demux->psz_demux, "ts"))
        {
            msg_Dbg(demux, "dynamic payload format %s specified by demux", demux->psz_demux);
            pt.init = demux_init;
            pt.destroy = stream_destroy;
            pt.decode = stream_decode;
            pt.frequency = 90000;
        ...
        if (ptype >= 96)
        {
            ...
            else if (!strcmp(dynamic, "theora"))
            {
                ...
                msg_Err(demux, "unknown dynamic payload format `%s' " "specified", dynamic);
            free(dynamic);
        }
