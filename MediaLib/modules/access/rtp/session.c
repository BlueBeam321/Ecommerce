...
void rtp_queue(demux_t *demux, rtp_session_t *session, block_t *block)
{
    ...
    if ((delta_seq > 0) ? (delta_seq > p_sys->max_dropout) : (-delta_seq > p_sys->max_misorder))
    {
 ...
 */
bool rtp_dequeue(demux_t *demux, const rtp_session_t *session, mtime_t * deadlinep)
{
    mtime_t now = mdate();
    bool pending = false;
...
static void rtp_decode(demux_t *demux, const rtp_session_t *session, rtp_source_t *src)
{
    ...
    if (delta_seq != 0)
    {
        ...
        {   /* Trash too late packets (and PIM Assert duplicates) */
            msg_Dbg(demux, "ignoring late packet (sequence: %"PRIu16")", rtp_seq(block));
            goto drop;
        }
    ...
    if (pt == NULL)
    {
        msg_Dbg(demux, "unknown payload (%"PRIu8")", rtp_ptype(block));
        goto drop;
    }
    ...
    block->i_pts = src->ref_ntp + CLOCK_FREQ * (int32_t)(timestamp - src->ref_rtp) / pt->frequency;
