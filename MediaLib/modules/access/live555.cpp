...
#define MODULE_NAME     access_demux_rtsp
#define MODULE_STRING   "access_demux_rtsp"
...
createnew:
    ...
    p_sys->rtsp = new RTSPClientVlc( *p_sys->env, p_sys->psz_pl_url,
                                     var_InheritInteger( p_demux, "verbose" ) > 1 ? 1 : 0,
                                     "LibVLC/3.0", i_http_port, p_sys );
    if( !p_sys->rtsp )
    {
                   ...
                   if( i_extraTot > 0 )
                    {
                        ...
                        tk->fmt.p_extra = malloc(i_extraTot);
                       if( p_extra1 )
                       {
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_SET_RATE:
        {
             ...
             *  may  the range of scale values that it supports.
                                      ...
                                      unsigned int& configSize )
{
    ...
    if (configSize > headerSkip && ((uint8_t*)p_cfg)[3] == 1)
    {
        ...
        p_extra = (uint8_t*)malloc(configSize);
