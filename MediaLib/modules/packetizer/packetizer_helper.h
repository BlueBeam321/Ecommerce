...
static inline block_t *packetizer_Packetize(packetizer_t *p_pack, block_t **pp_block)
{
    ...
    for (;;)
    {
        ...
        case STATE_NOSYNC:
            ...
            p_pack->i_offset = 1; /* To find next start-code */
            /* fall-through */
        ...
        case STATE_NEXT_SYNC:
                ...
                p_pack->pf_startcode_helper, NULL))
            {
                ...
                /* When flushing and we don't find a start-code, suppose that
                 * the data extend up to the end */
                block_ChainProperties(p_pack->bytestream.p_chain, NULL, &p_pack->i_offset, NULL);
            ...
            block_GetBytes(&p_pack->bytestream, &p_pic->p_buffer[p_pack->i_au_prepend], p_pic->i_buffer - p_pack->i_au_prepend);
...
static inline void packetizer_Header(packetizer_t *p_pack, const uint8_t *p_header, int i_header)
{
