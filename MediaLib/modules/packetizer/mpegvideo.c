...
#include "common/modules/codec/cc.h"
#include "packetizer_helper.h"
#include "common/modules/packetizer/startcode_helper.h"

#include <limits.h>

#define MODULE_NAME     packetizer_mpegvideo
#define MODULE_STRING   "packetizer_mpegvideo"
...
struct decoder_sys_t
{
    ...
    //block_fifo_t* p_pending_frame_fifo; // Added by K.C.Y
...
static int Open(vlc_object_t *p_this)
{
    ...
    p_sys->b_waiting_iframe = p_sys->b_sync_on_intra_frame = var_CreateGetBool(p_dec, "packetizer-mpegvideo-sync-iframe");
    if (p_sys->b_sync_on_intra_frame)
        msg_Dbg(p_dec, "syncing on intra frame now");

    //p_sys->p_pending_frame_fifo = block_FifoNew(); // Added by K.C.Y
...
static void Close(vlc_object_t *p_this)
{
    ...
    //if (p_sys->p_pending_frame_fifo) /* Added by K.C.Y */
    //    block_FifoRelease(p_sys->p_pending_frame_fifo);
...
static block_t *GetCc(decoder_t *p_dec, decoder_cc_desc_t *p_desc)
{
    ...
    if (p_cc)
    {
        ...
        p_cc->i_dts = p_cc->i_pts = p_sys->cc.b_reorder ? p_sys->i_cc_pts : p_sys->i_cc_dts;
...
static void PacketizeReset(void *p_private, bool b_broken)
{
    ...
    date_Set(&p_sys->dts, VLC_TS_INVALID);
    date_Set(&p_sys->prev_iframe_dts, VLC_TS_INVALID);
    p_sys->i_dts = p_sys->i_pts = p_sys->i_last_ref_pts = VLC_TS_INVALID;
    p_sys->b_waiting_iframe = p_sys->b_sync_on_intra_frame;
    p_sys->i_prev_temporal_ref = 2048;

    //if (p_sys->p_pending_frame_fifo) // Added by K.C.Y
    //    block_FifoEmpty(p_sys->p_pending_frame_fifo);
...
static block_t *ParseMPEGBlock(decoder_t *p_dec, block_t *p_frag)
{
        ...
        (startcode == PICTURE_STARTCODE || startcode > SLICE_STARTCODE_LAST))
    {
        ...
        if (!p_sys->b_seq_progressive)
        {
            if (p_sys->i_picture_structure < 0x03)
            {
                ...
                p_pic->i_flags |= (p_sys->i_picture_structure == 0x01) ? BLOCK_FLAG_TOP_FIELD_FIRST : BLOCK_FLAG_BOTTOM_FIELD_FIRST;
            }
            else /* if( p_sys->i_picture_structure == 0x03 ) */
                p_pic->i_flags |= (p_sys->i_top_field_first) ? BLOCK_FLAG_TOP_FIELD_FIRST : BLOCK_FLAG_BOTTOM_FIELD_FIRST;
        }
        ...
        if (unlikely(p_dec->fmt_in.i_original_fourcc == VLC_FOURCC('D', 'V', 'R', ' ')))
        {
            ...
            if (p_sys->i_picture_structure != 0x03)
            {
                ...
                if ((p_sys->i_picture_structure >> 1) != !p_sys->i_top_field_first && b_first_xmited)
    ...
    /*if (p_pic)
    {
        mtime_t last_frame_pts = p_pic->i_pts;
        size_t count = block_FifoCount(p_sys->p_pending_frame_fifo);
        if (p_pic->i_flags & (BLOCK_FLAG_TYPE_I | BLOCK_FLAG_TYPE_P))
        {
            block_t* p_block = count > 0 ? block_FifoShow(p_sys->p_pending_frame_fifo) : NULL;
            while (p_block)
            {
                if (p_block->i_flags & (BLOCK_FLAG_TYPE_I | BLOCK_FLAG_TYPE_P))
                {
                    if (p_block->i_pts == VLC_TS_INVALID)
                        p_block->i_pts = p_sys->i_last_frame_pts + (CLOCK_FREQ * p_sys->i_frame_rate_base / p_sys->i_frame_rate);
                    break;
                }
                p_block = p_block->p_next;
            }
            p_block = count > 0 ? block_FifoGet(p_sys->p_pending_frame_fifo) : NULL;
            block_FifoPut(p_sys->p_pending_frame_fifo, p_pic);
            p_pic = p_block;
        }
        else
        {
            if (count > 0)
            {
                block_t* p_block = block_FifoShow(p_sys->p_pending_frame_fifo);
                if (p_block->i_flags & BLOCK_FLAG_TYPE_B)
                    p_block = block_FifoGet(p_sys->p_pending_frame_fifo);
                else
                {
                    uint8_t ip_found = 0;
                    p_block = p_block->p_next;
        ...
        p_sys->i_last_frame_pts = last_frame_pts;
    }*/
    ...
    else if (startcode == SEQUENCE_HEADER_STARTCODE && p_frag->i_buffer >= 8)
    {
        ...
        p_dec->fmt_out.video.i_visible_width = (p_frag->p_buffer[4] << 4) | (p_frag->p_buffer[5] >> 4);
        p_dec->fmt_out.video.i_width = (p_dec->fmt_out.video.i_visible_width + 0x0F) & ~0x0F;
        p_dec->fmt_out.video.i_visible_height = ((p_frag->p_buffer[5] & 0x0f) << 8) | p_frag->p_buffer[6];
        if (p_sys->b_seq_progressive)
    ...
    else if (startcode == EXTENSION_STARTCODE && p_frag->i_buffer > 4)
    {
        ...
        if (extid == SEQUENCE_EXTENSION_ID)
        {
            ...
            {
                p_sys->b_seq_progressive = p_frag->p_buffer[5] & 0x08 ? true : false;
                p_sys->b_low_delay = p_frag->p_buffer[9] & 0x80 ? true : false;
            }
    ...
    else if (startcode == USER_DATA_STARTCODE && p_frag->i_buffer > 8)
    {
            ...
            p_dec->fmt_in.video.multiview_mode == MULTIVIEW_2D)
        {
            ...
            cc_ProbeAndExtract(&p_sys->cc, p_sys->i_top_field_first, &p_frag->p_buffer[4], p_frag->i_buffer - 4);
    }
    ...
    else if (startcode >= SLICE_STARTCODE_FIRST && startcode <= SLICE_STARTCODE_LAST)
    {
        /* Slice start code */
