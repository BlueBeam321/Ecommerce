...
#include "common/modules/codec/avcodec/chroma.h" // Chroma Avutil <-> VLC conversion

#define MODULE_NAME     vchroma_ffmpeg
#define MODULE_STRING   "vchroma_ffmpeg"
...
static void ExtractA( picture_t *p_dst, const picture_t * p_src,
                      unsigned offset )
{
    plane_t *d = &p_dst->p[0];
...
static void InjectA( picture_t *p_dst, const picture_t * p_src,
                     unsigned offset )
{
    plane_t *d = &p_dst->p[0];
    const plane_t *s = &p_src->p[0];
