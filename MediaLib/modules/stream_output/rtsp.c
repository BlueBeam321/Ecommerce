                        ...
                        const httpd_message_t *query )
{
    ...
    char control[256/*sizeof("rtsp://[]:12345") + NI_MAXNUMERICHOST + strlen(rtsp->psz_path)*/];
    ...
    httpd_MsgAdd(answer, "Server", "VLC/3.0.6");
    ...
    switch( query->i_type )
    {
        ...
        case HTTPD_MSG_PLAY:
        {
            ...
            if( ses != NULL )
            {
                char info[1024/*ses->trackc * (strlen(control) + TRACK_PATH_SIZE + sizeof("url=;seq=65535;rtptime=4294967295, ") - 1) + 1*/];
                size_t infolen = 0;
                RtspClientAlive(ses);
