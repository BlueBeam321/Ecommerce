...
#define MODULE_NAME     sout_rtp
#define MODULE_STRING   "sout_rtp"
                                  ...
                                  const es_format_t *p_fmt )
{
    ...
    if( p_sys->psz_destination != NULL )
    {
        ...
        switch (p_sys->proto)
        {
            ...
            case IPPROTO_TCP:
                ...
                if (vlc_clone(&id->listen.thread, rtp_listen_thread, id, "rtp_listen_thread", VLC_THREAD_PRIORITY_LOW))
                {
            ...
            default:
            {
                int fd = net_ConnectDgram(p_stream, p_sys->psz_destination, i_port, -1, p_sys->proto);
                if (fd == -1)
                {
    ...
    if( unlikely(id->p_fifo == NULL) )
        goto error;
    if (vlc_clone(&id->thread, ThreadSend, id, "ThreadSend", VLC_THREAD_PRIORITY_HIGHEST))
    {
...
# define EWOULDBLOCK  WSAEWOULDBLOCK
    ...
    for (;;)
    {
        ...
        int deadv[64/*id->sinkc ? id->sinkc : 1*/]; /* Dead sockets list */
