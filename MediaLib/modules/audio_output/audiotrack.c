...
#define MODULE_NAME     aout_audiotrack
#define MODULE_STRING   "aout_audiotrack"
 ...
 * Android resampler will be used to downmix to stereo. It may fails cleanly,
 * and this module will be able to recover and fallback to stereo. Finally, in
 * some rare cases, it may crash during init or while resampling. Because of
...
static int
TimeGet( audio_output_t *p_aout, mtime_t * p_delay )
{
    aout_sys_t *p_sys = p_aout->sys;
...
static int
Start( audio_output_t *p_aout, audio_sample_format_t * p_fmt )
{
    aout_sys_t *p_sys = p_aout->sys;
    JNIEnv *env;
    int i_ret;
    ...
    if (vlc_clone(&p_sys->thread, AudioTrack_Thread, p_aout, "aout_audiotrack", VLC_THREAD_PRIORITY_LOW))
    {
                          ...
                          bool b_force )
{
    ...
    assert( i_audiotrack_pos <= p_sys->i_samples_written ); // FIXME K.C.Y
