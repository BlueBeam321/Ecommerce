...
#define MODULE_NAME     aout_waveout
#define MODULE_STRING   "aout_waveout"

#include "common/modules/audio_output/windows_audio_common.h"
                                 ...
                                 char ***values, char ***descs )
{
    ...
    *values = malloc((nb_devices + 1) * sizeof(char *));
    *descs = malloc((nb_devices + 1) * sizeof(char *));
...
static int WaveoutVolumeSet( audio_output_t *p_aout, float volume )
{
    ...
    else
    {
        ...
        uint32_t vol = lroundf( volume * (float)(0x7fff));
...
static int WaveoutMuteSet( audio_output_t * p_aout, bool mute )
{
    ...
    else
    {
        ...
        const HWAVEOUT hwo = sys->h_waveout;
        uint32_t vol = mute ? 0 : lroundf( sys->f_volume * (float)(0x7fff) );
