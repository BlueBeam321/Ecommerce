...
static char *mp4_getstringz( uint8_t ** in, uint64_t * size )
{
                                          ...
                                          uint64_t readsize )
{
    ...
    if( unlikely(readsize < headersize) || unlikely(readsize > LONG_MAX) )
        return NULL;
...
static int MP4_ReadBox_tkhd(  stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( likely(scale[0] > 0 && scale[1] > 0) )
    {
        rotation = atan2(conv_fx(matrix[1]) / scale[1],
            conv_fx(matrix[0]) / scale[0]) * 180 / 3.14159265358979323846/*M_PI*/;
        if (rotation < 0)
            rotation += 360.;
    }
...
static uint64_t MP4_ReadLengthDescriptor( uint8_t ** bufp,
                                          uint64_t * lenp )
{
...
static const struct
{
    ...
    uint32_t i_parent; /* set parent to , duplicating if needed; 0 for any */
} MP4_Box_Function [] =
{
    /* Containers */
