...
void EbmlParser::reconstruct( EbmlStream* es, EbmlElement* el_start, demux_t* p_demux )
{
    ...
    this->mi_level = 1;
    this->m_got = NULL;
    this->mi_user_level = 1;
    this->mb_keep = false;
    this->mb_dummy = var_InheritBool(p_demux, "mkv-use-dummy");
    memset(this->m_el, 0, sizeof(*this->m_el) * M_EL_MAXSIZE);
    this->m_el[0] = el_start;
#endif
}
...
next:
    ...
    else if( m_el[mi_level]->IsDummy() && !mb_dummy )
    {
              ...
              m_el[mi_level]->GetEndPosition() <= m_el[mi_level-1]->GetEndPosition() ) )
        {
            ...
            msg_Warn( p_demux, "Dummy element found %llu... skipping it",
        ...
        else
        {
            ...
            msg_Err( p_demux,
                     "Dummy element too large or misplaced at %llu... skipping to next upper element",
                     m_el[mi_level]->GetElementPosition() );
