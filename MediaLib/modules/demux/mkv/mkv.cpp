...
#define MODULE_NAME     demux_mkv
#define MODULE_STRING   "demux_mkv"
                    ...
                    if (strlen(psz_file) > 4)
                    {
                            ...
                            !strcasecmp(s_filename.c_str() + s_filename.length() - 4, ".mka"))
                        {
                            ...
                            if (p_file_stream &&
                                ...
                                && p_peek[0] == 0x1a && p_peek[1] == 0x45 && p_peek[2] == 0xdf && p_peek[3] == 0xa3)
                                file_ok = true;
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_SET_TIME:
            ...
            b = va_arg( args, int ); /* precise? */
            msg_Dbg(p_demux,"SET_TIME to %lld", i64 );
...
static int Seek( demux_t *p_demux, mtime_t i_mk_date, double f_percent, virtual_chapter_c *p_vchapter, bool b_precise )
{
    ...
    if( f_percent < 0 ) msg_Dbg( p_demux, "seek request to i_pos = %lld", i_mk_date );
...
static int Demux(demux_t *p_demux)
{
...

    {
        ...
        if (track.i_skip_until_fpos != std::numeric_limits<uint64_t>::max())
        {
            ...
            if (block)
                block_fpos = block->GetElementPosition();
            else
                block_fpos = simpleblock->GetElementPosition();
    ...
    /* set pts */
    {
        ...
        if (simpleblock != NULL)
            p_sys->i_pts += simpleblock->GlobalTimecode() / INT64_C(1000);
        else
            p_sys->i_pts += block->GlobalTimecode() / INT64_C(1000);
    }
