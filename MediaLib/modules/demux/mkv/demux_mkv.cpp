...
void event_thread_t::SetPci(const pci_t *data)
{
    ...
    {
        b_abort = false;
        is_running = !vlc_clone(&thread, EventThread, this, "demux_mkv_event", VLC_THREAD_PRIORITY_LOW);
    }
}
...
demux_sys_t::~demux_sys_t()
{
    ...
    while (titles.size())
    {
        vlc_input_title_Delete(titles.back());
        titles.pop_back();
    }
...
bool demux_sys_t::AnalyseAllSegmentsFound( demux_t *p_demux, matroska_stream_c *p_stream1, bool b_initial )
{
    ...
    {
        msg_Err(p_demux, "matroska file needs version %lld but only versions 1 & 2 supported", uint64(doc_read_version));
        return false;
    }
...
bool demux_sys_t::PreloadLinked()
{
    ...
    msg_Dbg(&demuxer, "NEW START CHAPTER uid=%lld", p_current_vsegment->p_current_vchapter && p_current_vsegment->p_current_vchapter->p_chapter ?
            ...
            for (j = 0; j < p_vseg->Editions()->size(); j++)
            {
                ...
                if (p_title->psz_name == NULL)
                {
                    ...
                    else
                    {
                        ...
                        for (size_t k = 0; k < tags.size(); k++)
                        {
                            if (tags[k].i_tag_type == EDITION_UID && tags[k].i_uid == i_ed_uid)
                                for (size_t l = 0; l < tags[k].simple_tags.size(); l++)
                                {
                                    ...
                                    {
                                        msg_Dbg( &demuxer, "Using title \"%s\" from tag for edition %llu", st.value.c_str (), i_ed_uid );
                        ...
                        if (!p_title->psz_name && asprintf(&(p_title->psz_name), "%s %d", "Segment", (int)ij) == -1)
