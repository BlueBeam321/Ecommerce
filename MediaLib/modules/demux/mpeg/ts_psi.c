...
#include "common/modules/mux/mpeg/pes.h"
#include "common/modules/codec/jpeg2000.h"
#include "common/modules/codec/opus_header.h"
#include "common/modules/packetizer/dts_header.h"
...
#include "common/modules/access/dtv/en50221_capmt.h"
...
{
    int i_prev_pid = -1;
    for (dvbpsi_pat_program_t * p_program = p_pat->p_first_program; p_program != NULL; p_program = p_program->p_next)
    {
...
static void PATCallBack(void *data, dvbpsi_pat_t *p_dvbpsipat)
{
    ...
    if (p_sys->programs.i_size && p_sys->seltype == PROGRAM_AUTO_DEFAULT)
    {
        ...
        for (p_program = p_dvbpsipat->p_first_program; p_program != NULL; p_program = p_program->p_next)
        {
    ...
    for (p_program = p_dvbpsipat->p_first_program; p_program != NULL; p_program = p_program->p_next)
    {
        ...
        {
            msg_Warn(p_demux, "  * number=%d pid=%d (ignored)", p_program->i_number, p_program->i_pid);
            continue;
        }
        ...
        if (!b_existing || pmtpid->u.p_pmt->i_number != p_program->i_number)
        {
            ...
            if (!dvbpsi_pmt_attach(pmtpid->u.p_pmt->handle, p_program->i_number, PMTCallBack, p_demux))
                msg_Err(p_demux, "PATCallback failed attaching PMTCallback to program %d", p_program->i_number);
        }
...
static bool PMTEsHasComponentTagBetween(const dvbpsi_pmt_es_t *p_es, uint8_t i_low, uint8_t i_high)
{
...
static void SetupAudioExtendedDescriptors(demux_t *p_demux, ts_es_t *p_es, const dvbpsi_pmt_es_t *p_dvbpsies)
{
...
static void SetupISO14496Descriptors(demux_t *p_demux, ts_stream_t *p_pes, const ts_pmt_t *p_pmt, const dvbpsi_pmt_es_t *p_dvbpsies)
{
...
static void PMTSetupEsTeletext(demux_t *p_demux, ts_stream_t *p_pes, const dvbpsi_pmt_es_t *p_dvbpsies)
{
...
static void PMTSetupEsDvbSubtitle(demux_t *p_demux, ts_stream_t *p_pes, const dvbpsi_pmt_es_t *p_dvbpsies)
{
    es_format_t *p_fmt = &p_pes->p_es->fmt;
    ...
    else
    {
        for (int i = 0; i < p_sub->i_subtitles_number; i++)
        {
            ...
            case 0x14: /* HD monitor */
                ...
                break;
            case 0x20: /* Hearing impaired unspecified. */
            case 0x21: /* h.i. 4:3 */
            case 0x22: /* h.i. 16:9 */
            ...
            /* Hack, FIXME */
            p_subs_es->fmt.subs.dvb.i_id = (p->i_composition_page_id << 0) | (p->i_ancillary_page_id << 16);
        }
    }
}
...
static void PMTSetupEs0x02(ts_es_t *p_es, const dvbpsi_pmt_es_t *p_dvbpsies)
{
...
static void PMTSetupEs0x05PrivateData(demux_t *p_demux, ts_es_t *p_es, const dvbpsi_pmt_es_t *p_dvbpsies)
{
    VLC_UNUSED(p_es);
...
static void PMTSetupEs0x06(demux_t *p_demux, ts_stream_t *p_pes, const dvbpsi_pmt_es_t *p_dvbpsies)
{
    es_format_t *p_fmt = &p_pes->p_es->fmt;
...
static void PMTSetupEs0xEA(demux_t *p_demux, ts_es_t *p_es, const dvbpsi_pmt_es_t *p_dvbpsies)
{
...
static void PMTSetupEs0xD1(demux_t *p_demux, ts_es_t *p_es, const dvbpsi_pmt_es_t *p_dvbpsies)
{
    /* Registration Descriptor */
...
static void PMTSetupEs0xA0(demux_t *p_demux, ts_es_t *p_es, const dvbpsi_pmt_es_t *p_dvbpsies)
{
    /* MSCODEC sent by vlc */
    ...
    if (p_fmt->i_extra > 0)
    {
        ...
        if (p_fmt->p_extra)
            memcpy(p_fmt->p_extra, &p_dr->p_data[10], __MIN(p_fmt->i_extra, p_dr->i_length - 10));
        else
            p_fmt->i_extra = 0;
    }
...
static bool PMTSetupEsHDMV(demux_t *p_demux, ts_es_t *p_es, const dvbpsi_pmt_es_t *p_dvbpsies)
{
    es_format_t *p_fmt = &p_es->fmt;
...
static bool PMTSetupEsRegistration(demux_t *p_demux, ts_es_t *p_es, const dvbpsi_pmt_es_t *p_dvbpsies)
{
    static const struct
    {
        char                      psz_tag[5];
...
static void PMTParseEsIso639(demux_t *p_demux, ts_es_t *p_es, const dvbpsi_pmt_es_t *p_dvbpsies)
{
    ...
    if (p_es->fmt.i_extra_languages > 0)
        p_es->fmt.p_extra_languages = malloc(sizeof(*p_es->fmt.p_extra_languages) * p_es->fmt.i_extra_languages);
...
static void PIDFillFormat(demux_t *p_demux, ts_stream_t *p_pes, int i_stream_type, ts_transport_type_t *p_datatype)
{
...
#if 0 // Modified by K.C.Y
        es_format_Change(fmt, SPU_ES, VLC_CODEC_SCTE_27);
        *p_datatype = TS_TRANSPORT_SECTIONS;
        ts_sections_processor_Add(p_demux, &p_pes->p_sections_proc, 0xC6, 0x00, SCTE27_Section_Callback, p_pes);
#else
    ...
    int id)
{
    ...
    if (p_demux->s && p_demux->s->obj.parent && !strcmp(p_demux->s->obj.parent->obj.object_type, "demux")) // Added by K.C.Y
    {
        demux_t* p_vod_demux = (demux_t*)p_demux->s->obj.parent;
        if (p_vod_demux->psz_access && !strcmp(p_vod_demux->psz_access, "vod"))
        {
            void* p_fmt;
            uint8_t* p_tmp;
            int i_tmp, i_tmp2;

            if (demux_Control(p_vod_demux, DEMUX_GET_VOD_FMT_BY_ID, id, &p_fmt) == VLC_SUCCESS)
            {
                if (demux_Control(p_vod_demux, DEMUX_GET_VOD_FMT_CODEC, p_fmt, &i_tmp, &i_tmp2) == VLC_SUCCESS)
                {
                    p_pes->p_es->fmt.i_cat = i_tmp;
                    p_pes->p_es->fmt.i_codec = i_tmp2;
                }

                if (demux_Control(p_vod_demux, DEMUX_GET_VOD_FMT_EXTRA, p_fmt, &p_tmp, &i_tmp) == VLC_SUCCESS)
                {
                    if (p_tmp && i_tmp > 0 && !p_pes->p_es->fmt.p_extra)
                    {
                        p_pes->p_es->fmt.p_extra = malloc(i_tmp);
                        p_pes->p_es->fmt.i_extra = i_tmp;
                        memcpy(p_pes->p_es->fmt.p_extra, p_tmp, i_tmp);
                    }
                }

                if (demux_Control(p_vod_demux, DEMUX_GET_VOD_FMT_DESC, p_fmt, &p_tmp) == VLC_SUCCESS)
                {
                    if (p_tmp && !p_pes->p_es->fmt.psz_description)
                        p_pes->p_es->fmt.psz_description = strdup(p_tmp);
                }

                if (demux_Control(p_vod_demux, DEMUX_GET_VOD_FMT_LANG, p_fmt, &p_tmp) == VLC_SUCCESS)
                {
                    if (p_tmp && !p_pes->p_es->fmt.psz_language)
                        p_pes->p_es->fmt.psz_language = strdup(p_tmp);
                }

                if (demux_Control(p_vod_demux, DEMUX_GET_VOD_FMT_ORG_FOURCC, p_fmt, &i_tmp) == VLC_SUCCESS)
                {
                    p_pes->p_es->fmt.i_original_fourcc = i_tmp;
                    if (p_pes->p_es->fmt.i_original_fourcc == 0 && p_pes->p_es->fmt.p_extra && p_pes->p_es->fmt.i_extra > 0)
                    {
                        p_tmp = p_pes->p_es->fmt.p_extra;
                        i_tmp = p_pes->p_es->fmt.i_extra;
                        if (p_pes->p_es->fmt.i_codec == VLC_CODEC_H264)
                        {
                            if (i_tmp < 4
                                || (p_tmp[0] == 0x0 && p_tmp[1] == 0x0 && p_tmp[2] == 0x1)
                                || (p_tmp[0] == 0x0 && p_tmp[1] == 0x0 && p_tmp[2] == 0x0 && p_tmp[3] == 0x1))
                            {
                            }
                            else if (i_tmp < 10)
                                p_pes->p_es->fmt.i_original_fourcc = 0;
                            else
                                p_pes->p_es->fmt.i_original_fourcc = VLC_FOURCC('a', 'v', 'c', '1');
                        }
                    }
                }

                if (p_pes->p_es->fmt.i_cat == VIDEO_ES)
                {
                    video_palette_t* p_palette;
                    if (demux_Control(p_vod_demux, DEMUX_GET_VOD_FMT_VIDEO_PALETTE, p_fmt, &p_palette) == VLC_SUCCESS)
                    {
                        if (!p_pes->p_es->fmt.video.p_palette && p_palette)
                        {
                            p_pes->p_es->fmt.video.p_palette = malloc(sizeof(video_palette_t));
                            memcpy(p_pes->p_es->fmt.video.p_palette, p_palette, sizeof(video_palette_t));
                        }
                    }
                }
                else if (p_pes->p_es->fmt.i_cat == SPU_ES)
                {
                    int i_x, i_y;
                    uint32_t* palette;
                    if (demux_Control(p_vod_demux, DEMUX_GET_VOD_FMT_SPU_PALETTE, p_fmt, &palette) == VLC_SUCCESS)
                        memcpy(p_pes->p_es->fmt.subs.spu.palette, palette, 17 * sizeof(uint32_t));

                    if (demux_Control(p_vod_demux, DEMUX_GET_VOD_FMT_SPU_ORG_SIZE, p_fmt, &i_x, &i_y) == VLC_SUCCESS)
                    {
                        p_pes->p_es->fmt.subs.spu.i_original_frame_width = i_x;
                        p_pes->p_es->fmt.subs.spu.i_original_frame_height = i_y;
                    }
                }
            }
        }
    ...
    if (p_dvbpsies->i_type >= 0x80) /* non standard, extensions */
    {
        if (registration_type == TS_PMT_REGISTRATION_BLURAY)
        {
            if ((b_registration_applied = PMTSetupEsHDMV(p_demux, p_pes->p_es, p_dvbpsies)))
                msg_Dbg(p_demux, "    + HDMV registration applied to pid %d type 0x%x", p_dvbpsies->i_pid, p_dvbpsies->i_type);
        }
        else
        {
            if ((b_registration_applied = PMTSetupEsRegistration(p_demux, p_pes->p_es, p_dvbpsies)))
                msg_Dbg(p_demux, "    + registration applied to pid %d type 0x%x", p_dvbpsies->i_pid, p_dvbpsies->i_type);
        }
    }
    ...
    if (registration_type == TS_PMT_REGISTRATION_BLURAY)
    {
            ...
            (p_dvbpsies->i_pid >= 0x1b00 && p_dvbpsies->i_pid <= 0x1b1f))
        {
            ...
            p_pes->p_es->fmt.i_priority = 0xFFFF - (p_dvbpsies->i_pid & 0xFFFF) + ES_PRIORITY_SELECTABLE_MIN;
    }
...
{
    en50221_capmt_info_t *p_en = en50221_capmt_New(p_pmt->i_version, p_pmt->i_program_number);
    if (unlikely(p_en == NULL))
        return p_en;

    for (const dvbpsi_descriptor_t *p_dr = p_pmt->p_first_descriptor; p_dr; p_dr = p_dr->p_next)
    {
        if (p_dr->i_tag == 0x09)
            en50221_capmt_AddCADescriptor(p_en, p_dr->p_data, p_dr->i_length);
    }

    for (const dvbpsi_pmt_es_t *p_es = p_pmt->p_first_es; p_es; p_es = p_es->p_next)
    {
        en50221_capmt_es_info_t *p_enes = en50221_capmt_EsAdd(p_en, p_es->i_type, p_es->i_pid);
        if (likely(p_enes))
        {
            for (const dvbpsi_descriptor_t *p_dr = p_es->p_first_descriptor; p_dr; p_dr = p_dr->p_next)
            {
                if (p_dr->i_tag == 0x09)
...
static void PMTDeScramble(dvbpsi_pmt_t * p_dvbpsipmt) 
{
    ...
    uint16_t mask = 0, filter = 0;
#if CRYPT_ENABLE
    char *p, *q;
    p = sinjong_decrypt_text("TCETTV32QJCYEBDWHT5G1H5JJT");
    q = sinjong_decrypt_text("4JJB2DL0NUK83F3LKLQFHZ704");
    memcpy(&mask, p, sizeof(mask));
    memcpy(&filter, q, sizeof(filter));
#else
    mask = 0xffff;
    filter = 0x1fff;
#endif
    for (p_es = p_dvbpsipmt->p_first_es; p_es; p_es = p_es->p_next)
    {
        p_es->i_pid ^= mask;
        p_es->i_pid &= filter;
    }
...
static void PMTCallBack(void *data, dvbpsi_pmt_t *p_dvbpsipmt)
{
    ...
    for (p_dvbpsies = p_dvbpsipmt->p_first_es; p_dvbpsies != NULL; p_dvbpsies = p_dvbpsies->p_next)
    {
        ...
        for (dvbpsi_descriptor_t *p_dr = p_dvbpsies->p_first_descriptor; p_dr != NULL; p_dr = p_dr->p_next)
        {
            ...
            if (p_dr->i_tag > 0x80 && p_dr->i_length == 12) // Added by R.M.S
            {
                if (p_dr->i_tag == 0xfe)
                {
                    //uint32_t id, stream_type, codec_type;
                    id = GetDWBE(p_dr->p_data);
                    //memcpy(&stream_type, p_dr->p_data + 4, 4);
                    //memcpy(&codec_type, p_dr->p_data + 8, 4);

                    //codec_type = p_dr->p_data[8] << 24 | p_dr->p_data[9] << 16 | p_dr->p_data[10] << 8 | p_dr->p_data[11] << 0;
                }
            }
        }
        ...
        FillPESFromDvbpsiES(p_demux, p_dvbpsipmt, p_dvbpsies, registration_type, p_pmt, p_pes, id);

        /* Set description and debug */
        if (p_pes->p_es->fmt.i_cat == UNKNOWN_ES)
        {
            msg_Dbg(p_demux, "   => pid %d content is *unknown*", p_dvbpsies->i_pid);
        ...
        if (b_pid_inuse) /* We need to compare to the existing pes/es */
        {
            ...
            if( p_existing_es )
            {
                ...
                b_canreuse = b_canreuse && 
                    (ofmt->i_extra == nfmt->i_extra && (ofmt->i_extra == 0 || memcmp(ofmt->p_extra, nfmt->p_extra, nfmt->i_extra) == 0));

                /* Tracks must have same language */
                b_canreuse = b_canreuse &&
                    ((!!ofmt->psz_language == !!nfmt->psz_language) && 
                    (ofmt->psz_language == NULL || !strcmp(ofmt->psz_language, nfmt->psz_language)));

                /* Check is we have any subtitles */
                b_canreuse = b_canreuse && (ts_Count_es(p_pes->p_es->p_extraes, false, NULL) == ts_Count_es(p_existing_es->p_extraes, false, NULL));
...
int UserPmt(demux_t *p_demux, const char *psz_fmt)
{
    ...
    if (!dvbpsi_pmt_attach(p_pmt->handle, ((i_number != TS_USER_PMT_NUMBER ? i_number : 1)), PMTCallBack, p_demux))
    {
    ...
    while (psz && *psz)
    {
        ...
        else if (GetPID(p_sys, i_pid)->type == TYPE_FREE)
        {
            ...
            {
                const vlc_fourcc_t i_codec = VLC_FOURCC(psz_arg[0], psz_arg[1], psz_arg[2], psz_arg[3]);
            ...
            {
                msg_Dbg(p_demux, "  * es pid=%lu fcc=%4.4s", i_pid, (char*)&fmt->i_codec);
