...
static void EITCallBack( demux_t *p_demux, dvbpsi_eit_t *p_eit )
{
    ...
    if (p_eit->i_table_id == 0x4e || p_eit->i_table_id == 0x4f)
    {
        dvbpsi_eit_delete(p_eit);
        return;
    }
    ...
    for( p_evt = p_eit->p_first_event; p_evt; p_evt = p_evt->p_next )
    {
        ...
        int i_style_len = 0, i_title_len = 0;
        bool b_userdr_find = 0;
        for( p_dr = p_evt->p_first_descriptor; p_dr; p_dr = p_dr->p_next )
        {
            ...
            case 0x80:
            {
                dvbpsi_user_event_dr_t *pE = dvbpsi_DecodeUserEventDr(p_dr);
                if (!b_userdr_find)
                {
#if 0
                    p_epgevt->psz_name = malloc(pE->i_event_name_length);
                    p_epgevt->psz_short_description = malloc(pE->i_event_name_length);
                    p_epgevt->psz_description = malloc(pE->i_text_length + 2);

                    memset(p_epgevt->psz_name, 0, pE->i_event_name_length); //style
                    memset(p_epgevt->psz_short_description, 0, pE->i_event_name_length); //title
                    memset(p_epgevt->psz_description, 0, pE->i_text_length + 2); //content
                    /**
                    * |---1byte----|---2byte------|----nbyte----|----mbyte------|
                    * |---marker---|---style len--|----style----|----title------|
                    */
                    i_style_len = ((uint16_t*)(pE->i_event_name + 1))[0];
                    memcpy(p_epgevt->psz_name, pE->i_event_name + 3, i_style_len); //style, Remove Mark
                    memcpy(p_epgevt->psz_short_description, pE->i_event_name + 3 + i_style_len, pE->i_event_name_length - 3 - i_style_len); //style, Remove Mark
                    memcpy(p_epgevt->psz_description, pE->i_text + 1, pE->i_text_length - 1); //content
#else
                    i_style_len = ((uint16_t*)(pE->i_event_name + 1))[0];

                    p_epgevt->psz_name = FromCharset("UTF-16LE", pE->i_event_name + 3, i_style_len);
                    p_epgevt->psz_short_description = FromCharset("UTF-16LE", pE->i_event_name + 3 + i_style_len, pE->i_event_name_length - 3 - i_style_len); //style, Remove Mark
                    p_epgevt->psz_description = FromCharset("UTF-16LE", pE->i_text + 1, pE->i_text_length - 1);
#endif
                    b_userdr_find = true;
                }
                break;
