...
#include "common/modules/mux/mpeg/dvbpsi_compat.h" /* dvbpsi_messages */
...
static void ts_pes_ChainDelete_es( demux_t *p_demux, ts_es_t *p_es )
{
    while( p_es )
    {
        ...
        //msg_Dbg(NULL, "KCY: %s -- 1", __FUNCTION__);
        ts_pes_ChainDelete_es( p_demux, p_es->p_extraes );
        //msg_Dbg(NULL, "KCY: %s -- 2", __FUNCTION__);
        ts_pes_es_Clean( p_demux, p_es );
        //msg_Dbg(NULL, "KCY: %s -- 3", __FUNCTION__);
        free( p_es );
        p_es = p_next;
    }
}
...
void ts_stream_Del( demux_t *p_demux, ts_stream_t *pes )
{
    ...
    //msg_Dbg(NULL, "KCY: %s -- 4", __FUNCTION__);
    if( pes->prepcr.p_head )
        block_ChainRelease( pes->prepcr.p_head );

    //msg_Dbg(NULL, "KCY: %s -- 5", __FUNCTION__);
    free( pes );
}
