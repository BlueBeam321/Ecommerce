...
#include "common/modules/codec/scte18.h"
#include "../opus.h"
#include "common/modules/mux/mpeg/csa.h"
...
#define MODULE_NAME     demux_mpegts
#define MODULE_STRING   "demux_mpegts"
/*****************************************************************************
 * Module descriptor
 *****************************************************************************/
...
static int Open(vlc_object_t *p_this)
{
    ...
    vlc_stream_Control(p_sys->stream, STREAM_CAN_FASTSEEK, &p_sys->b_canfastseek);
...
static void Close(vlc_object_t *p_this)
{
    ...
    //msg_Dbg(NULL, "KCY: close ts -- 1");
    PIDRelease(p_demux, GetPID(p_sys, 0));

    //msg_Dbg(NULL, "KCY: close ts -- 2");
...
static int Demux(demux_t *p_demux)
{
    ...
    for (unsigned i_pkt = 0; i_pkt < p_sys->i_ts_read; i_pkt++)
    {
        ...
        if (p_sys->b_start_record)
        {
            ...
            vlc_stream_Control(p_sys->stream, STREAM_SET_RECORD_STATE, true, "ts");
            p_sys->b_start_record = false;
        }
        ...
        {
            msg_Dbg(p_demux, "transport_error_indicator set (pid=%d)", PIDGet(p_pkt));
            block_Release(p_pkt);
            continue;
        }
...
static inline void HasSelectedES(es_out_t *out, const ts_es_t *p_es, const ts_pmt_t *p_pmt, bool *pb_stream_selected)
{
    for (; p_es && !*pb_stream_selected; p_es = p_es->p_next)
    {
        if (p_es->id)
            es_out_Control(out, ES_OUT_GET_ES_STATE, p_es->id, pb_stream_selected);
...
static int Control(demux_t *p_demux, int i_query, va_list args)
{
    ...
    case DEMUX_GET_POSITION:
        ...
        {
            double i_length = TimeStampWrapAround(p_pmt->pcr.i_first, p_pmt->i_last_dts) - p_pmt->pcr.i_first;
            i_length += p_pmt->pcr.i_pcroffset;
            double i_pos = TimeStampWrapAround(p_pmt->pcr.i_first, p_pmt->pcr.i_current) - p_pmt->pcr.i_first;
            if (i_length > 0)
            {
                *pf = i_pos / i_length;
                return VLC_SUCCESS;
    ...
    case DEMUX_SET_POSITION:
        ...
        if (!p_sys->b_ignore_time_for_positions && b_bool && p_pmt &&
            ...
            p_pmt->pcr.i_current > -1)
        {
            int64_t i_length = TimeStampWrapAround(p_pmt->pcr.i_first, p_pmt->i_last_dts) - p_pmt->pcr.i_first;
        ...
        if (i64 > 0 && vlc_stream_Seek(p_sys->stream, (int64_t)(i64 * f)) == VLC_SUCCESS)
        {
            ReadyQueuesPostSeek(p_demux);
            return VLC_SUCCESS;
        }
    ...
    case DEMUX_GET_LENGTH:
        ...
        if (!p_sys->b_ignore_time_for_positions &&
            ...
            p_pmt->i_last_dts > 0)
        {
            int64_t i_start = (p_pmt->pcr.i_first > -1) ? p_pmt->pcr.i_first : TO_SCALE(p_pmt->pcr.i_first_dts);
    ...
    case DEMUX_GET_TITLE_INFO:
    {
        ...
        *va_arg(args, int*) = 0; /* Chapter offset */
        return vlc_stream_Control(p_sys->stream, STREAM_GET_TITLE_INFO, v, c);
    }
    ...
    case DEMUX_SET_SEEKPOINT:
        return vlc_stream_vaControl(p_sys->stream, STREAM_SET_SEEKPOINT, args);
    ...
    case DEMUX_SET_RECORD_STATE:
        ...
        if (!b_bool)
            vlc_stream_Control(p_sys->stream, STREAM_SET_RECORD_STATE, false);
    ...
    case DEMUX_GET_ATTACHMENTS:
    {
        ...
        *pi_int = 0;
        ...
        {
            for (vlc_dictionary_entry_t *p_entry = p_sys->attachments.p_entries[i]; p_entry; p_entry = p_entry->p_next)
            {
                msg_Err(p_demux, "GET ATTACHMENT %s", p_entry->psz_key);
                (*ppp_attach)[*pi_int] = vlc_input_attachment_Duplicate((input_attachment_t *)p_entry->p_value);
...
static block_t *Opus_Parse(demux_t *demux, block_t *block)
{
    ...
    while (len > 3 && ((buf[0] << 3) | (buf[1] >> 5)) == 0x3ff)
    {
        ...
        if (control_extension_flag && len)
        {
            ...
            if (l > len)
            {
                msg_Err(demux, "Invalid control extension length %d > %d", l, len);
                break;
            }
            buf += l;
            len -= l;
        ...
        {
            msg_Err(demux, "Invalid Opus AU size %d (PES %d)", au_size, len);
            break;
        }
...
static block_t * ConvertPESBlock(demux_t *p_demux, ts_es_t *p_es,
    size_t i_pes_size, uint8_t i_stream_id, block_t *p_block)
{
    if (!p_block)
        return NULL;
...
static void FeedNalHeader(block_t *p_pkt)
{
    uint8_t *p;
    uint32_t pos = 0;

    p = p_pkt->p_buffer;
    while (pos < p_pkt->i_buffer)
    {
        uint32_t size = 0;
        for (int i = 0; i < 4; i++)
        {
...
static void ParsePESDataChain(demux_t *p_demux, ts_pid_t *pid, block_t *p_pes)
{
    ...
    if (i_stream_id == 0xBF)
    {
        uint8_t* q = p_pes->p_buffer + 6;
        int32_t mark = GetDWBE(q);
        q += 4;
        if (mark == 'SYH2')
        {
            int i_ext_flags = 0;
            ...
            q += 8;
            // flags
            p_pes->i_flags = GetDWBE(q);
            q += 4;

            if (i_skip >= 52)
            {
                i_length = GetQWBE(q);
                i_length = TO_SCALE_NZ(i_length);
                q += 8;
                i_ext_flags = GetDWBE(q);
                //q += 4;
                //i_codec_desc = GetDWBE(q);
                // q += 4;
                //i_stream_id = GetDWBE(q);
                // q += 4;
            }

            if (i_ext_flags & /*BLOCK_EXT_FLAG_ONLY_DTS*/0x00000004)
                i_pts = i_dts;
        }
    }
    else if (i_stream_id == 0xBD)
    {
        uint8_t i_substream_id = p_pes->p_buffer[i_skip];
        if ((i_substream_id & 0xf8) == 0x88 || (i_substream_id & 0xf8) == 0x98 || (i_substream_id & 0xf0) == 0x80 || (i_substream_id & 0xf0) == 0xc0)
            i_skip += 4;
        else if ((i_substream_id & 0xf0) == 0xb0)
        {
            /* FIXME / untested ... no known decoder (at least not in VLC) */
        }
        else if ((i_substream_id & 0xe0) == 0x20 || (i_substream_id & 0xf0) == 0xa0)
            i_skip++;
    }
    else if (i_stream_id == 0xA0)
    {
        uint8_t i_substream_id = p_pes->p_buffer[i_skip] & 0x07;
        if (i_substream_id == 0)
            i_skip++;
        else if (i_substream_id == 1)
            i_skip--;
    }
    ...
    if (p_pes)
    {
        ...
        while (p_chain)
        {
            ...
            if (p_es->id && (p_pmt->pcr.i_current > -1 || p_pmt->pcr.b_disable))
            {
                ...
                ////////////////////For Vod//////////////////////////////////////
                //if (p_es->fmt.i_codec == VLC_CODEC_H264 && p_es->fmt.b_vod_stream)
                //    FeedNalHeader(p_block);
                /////////////////////////////////////////////////////////////////
...
static block_t* ReadTSPacket( demux_t *p_demux )
{
    ...
    if( p_pkt->p_buffer[0] != 0x47 )
    {
        ...
        for( ;; )
        {
            ...
            i_peek = vlc_stream_Peek(p_sys->stream, &p_peek, p_sys->i_packet_size * 10);
...
static inline void UpdateESScrambledState(es_out_t *out, const ts_es_t *p_es, bool b_scrambled)
{
    ...
    {
        if (p_es->id)
            es_out_Control(out, ES_OUT_SET_ES_SCRAMBLED_STATE, p_es->id, b_scrambled);
...
int ProbeStart(demux_t *p_demux, int i_program)
{
    ...
    } while (i_pos < i_stream_size && !b_found && i_probe_count < PROBE_MAX);
...
int ProbeEnd(demux_t *p_demux, int i_program)
{
    ...
    do
    {
        ...
        /* Go ahead one more chunk if end of file contained only stuffing packets */
        i_probe_count += PROBE_CHUNK_COUNT;
    } while (i_pos > 0 && !b_found && i_probe_count < PROBE_MAX);
...
static block_t* ProcessTSPacket(demux_t *p_demux, ts_pid_t *pid, block_t *p_pkt, int *pi_skip)
{
    ...
    *pi_skip = 4;
    ...
    if (pid->type == TYPE_STREAM) // Added by K.C.Y
    {
        ts_es_t* p_es = pid->u.p_stream->p_es;
        if (p_es->id)
        {
            bool selected = false;
            es_out_Control(p_demux->out, ES_OUT_GET_ES_STATE, p_es->id, &selected);
            if (!selected)
            {
                block_Release(p_pkt);
                return NULL;
    ...
    if (b_adaptation)
    {
        ...
        *pi_skip += 1 + p[4];
        ...
        else if (p[4] > 0)
        {
            ...
            if (b_discontinuity)
            {
                msg_Warn(p_demux, "discontinuity indicator (pid=%d) ", pid->i_pid);
    ...
    if (b_payload && p_demux->p_sys->b_cc_check)
    {
        ...
        else
        {
            if (pid->i_cc == 0xff)
            {
                msg_Dbg(p_demux, "first packet for pid=%d cc=0x%x", pid->i_pid, i_cc);
                pid->i_cc = i_cc;
            }
...
static bool GatherPESData(demux_t *p_demux, ts_pid_t *pid, block_t *p_pkt, size_t i_skip)
{
    ...
    for (bool b_first_sync_done = false; p_pkt;)
    {
        ...
        if (p_pes->gather.p_data == NULL && !b_first_sync_done && p_pkt->i_buffer >= 6)
        {
            ...
            /* Added by K.C.Y */
            uint8_t i_stream_id = p_pkt->p_buffer[3];
            if (i_stream_id == 0xBF)
            {
                uint8_t* q = &p_pkt->p_buffer[6];
                int32_t i_32 = GetDWBE(q);
                q += 4;
                ...
                if (i_32 == 'SYH2')
                {
                    i_32 = GetDWBE(q);
                    q += 4;
                    p_pes->gather.i_data_size = GetDWBE(q) + i_32 + 6;
                }
            }
        ...
        {
            assert(p_pes->gather.i_data_size > p_pes->gather.i_gathered || p_pes->gather.i_data_size == 0);
