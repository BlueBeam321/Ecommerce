...
#define MODULE_NAME     trender_freetype2
#define MODULE_STRING   "trender_freetype2"
...
//        }
        ...
        for( const vlc_fourcc_t *p_chroma = p_chroma_list; *p_chroma != 0; p_chroma++ )
        {
            ...
            if (*p_chroma == VLC_CODEC_YUVP)
                rv = RenderYUVP(p_filter, p_region_out, p_lines, &regionbbox, &paddedbbox, &bbox);
...
#elif defined( _WIN32 )
    const char *const ppsz_win32_default[] = { "Tahoma", "FangSong", "SimHei", "KaiTi" };
    p_sys->pf_get_family = Win32_GetFamily;
    p_sys->pf_get_fallbacks = Win32_GetFallbacks;
    p_sys->pf_select = Generic_Select;
    InitDefaultList(p_filter, ppsz_win32_default, sizeof(ppsz_win32_default) / sizeof(*ppsz_win32_default));
